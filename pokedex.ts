'use strict';

export const Pokedex: {[k: string]: ModdedSpeciesData} = {
  	arenay: {
		num: 50001,
		name: "Arenay",
		baseForme: "Normal",
		types: ["Normal"],
		gender: "N",
		baseStats: {hp: 90, atk: 70, def: 70, spa: 70, spd: 70, spe: 70},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		otherFormes: ["Arenay-Bug", "Arenay-Dark", "Arenay-Dragon", "Arenay-Electric", "Arenay-Fairy", "Arenay-Fighting", "Arenay-Fire", "Arenay-Flying", "Arenay-Ghost", "Arenay-Grass", "Arenay-Ground", "Arenay-Ice", "Arenay-Poison", "Arenay-Psychic", "Arenay-Rock", "Arenay-Steel", "Arenay-Water"],
		formeOrder: [
			"Arenay", "Arenay-Fire", "Arenay-Water", "Arenay-Grass", "Arenay-Flying", "Arenay-Steel", "Arenay-Electric", "Arenay-Ice", "Arenay-Psychic",
			"Arenay-Bug", "Arenay-Ground", "Arenay-Poison", "Arenay-Dragon", "Arenay-Fighting", "Arenay-Rock", "Arenay-Ghost", "Arenay-Dark", "Arenay-Fairy",
		],

	},
	arenayfire: {
		num: 50001,
		name: "Arenay-Fire",
		baseSpecies: "Arenay",
		forme: "Fire",
		types: ["Fire"],
		gender: "N",
		baseStats: {hp: 90, atk: 80, def: 80, spa: 110, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fire Cellulose"],
		changesFrom: "Arenay",
	},
	arenaywater: {
		num: 50001,
		name: "Arenay-Water",
		baseSpecies: "Arenay",
		forme: "Water",
		types: ["Water"],
		gender: "N",
		baseStats: {hp: 90, atk: 80, def: 110, spa: 80, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Water Cellulose"],
		changesFrom: "Arenay",
	},
	arenaygrass: {
		num: 50001,
		name: "Arenay-Grass",
		baseSpecies: "Arenay",
		forme: "Grass",
		types: ["Grass"],
		gender: "N",
		baseStats: {hp: 90, atk: 80, def: 80, spa: 80, spd: 80, spe: 110},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Grass Cellulose"],
		changesFrom: "Arenay",
	},
	arenayflying: {
		num: 50001,
		name: "Arenay-Flying",
		baseSpecies: "Arenay",
		forme: "Flying",
		types: ["Flying"],
		gender: "N",
		baseStats: {hp: 90, atk: 110, def: 80, spa: 80, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Flying Cellulose"],
		changesFrom: "Arenay",
	},
	arenaysteel: {
		num: 50001,
		name: "Arenay-Steel",
		baseSpecies: "Arenay",
		forme: "Steel",
		types: ["Steel"],
		gender: "N",
		baseStats: {hp: 90, atk: 80, def: 110, spa: 80, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Steel Cellulose"],
		changesFrom: "Arenay",
	},
	arenayelectric: {
		num: 50001,
		name: "Arenay-Electric",
		baseSpecies: "Arenay",
		forme: "Electric",
		types: ["Electric"],
		gender: "N",
		baseStats: {hp: 80, atk: 80, def: 80, spa: 110, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Electric Cellulose"],
		changesFrom: "Arenay",
	},
	arenayice: {
		num: 50001,
		name: "Arenay-Ice",
		baseSpecies: "Arenay",
		forme: "Ice",
		types: ["Ice"],
		gender: "N",
		baseStats: {hp: 80, atk: 80, def: 80, spa: 110, spd: 80, spe: 80},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ice Cellulose"],
		changesFrom: "Arenay",
	},
	arenaypsychic: {
		num: 50001,
		name: "Arenay-Psychic",
		baseSpecies: "Arenay",
		forme: "Psychic",
		types: ["Psychic"],
		gender: "N",
		baseStats: {hp: 90, atk: 70, def: 50, spa: 110, spd: 90, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Psychic Cellulose"],
		changesFrom: "Arenay",
	},
	arenaybug: {
		num: 50001,
		name: "Arenay-Bug",
		baseSpecies: "Arenay",
		forme: "Bug",
		types: ["Bug"],
		gender: "N",
		baseStats: {hp: 90, atk: 90, def: 60, spa: 70, spd: 60, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Bug Cellulose"],
		changesFrom: "Arenay",
	},
	arenayground: {
		num: 50001,
		name: "Arenay-Ground",
		baseSpecies: "Arenay",
		forme: "Ground",
		types: ["Ground"],
		gender: "N",
		baseStats: {hp: 90, atk: 100, def: 90, spa: 60, spd: 70, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ground Cellulose"],
		changesFrom: "Arenay",
	},
	arenaypoison: {
		num: 50001,
		name: "Arenay-Poison",
		baseSpecies: "Arenay",
		forme: "Poison",
		types: ["Poison"],
		gender: "N",
		baseStats: {hp: 90, atk: 100, def: 60, spa: 100, spd: 60, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Poison Cellulose"],
		changesFrom: "Arenay",
	},
	arenaydragon: {
		num: 50001,
		name: "Arenay-Dragon",
		baseSpecies: "Arenay",
		forme: "Dragon",
		types: ["Dragon"],
		gender: "N",
		baseStats: {hp: 90, atk: 90, def: 50, spa: 110, spd: 90, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dragon Cellulose"],
		changesFrom: "Arenay",
	},
	arenayfighting: {
		num: 50001,
		name: "Arenay-Fighting",
		baseSpecies: "Arenay",
		forme: "Fighting",
		types: ["Fighting"],
		gender: "N",
		baseStats: {hp: 90, atk: 120, def: 50, spa: 50, spd: 90, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fighting Cellulose"],
		changesFrom: "Arenay",
	},
	arenayrock: {
		num: 50001,
		name: "Arenay-Rock",
		baseSpecies: "Arenay",
		forme: "Rock",
		types: ["Rock"],
		gender: "N",
		baseStats: {hp: 90, atk: 90, def: 120, spa: 50, spd: 80, spe: 50},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Rock Cellulose"],
		changesFrom: "Arenay",
	},
	arenayghost: {
		num: 50001,
		name: "Arenay-Ghost",
		baseSpecies: "Arenay",
		forme: "Ghost",
		types: ["Ghost"],
		gender: "N",
		baseStats: {hp: 90, atk: 80, def: 60, spa: 110, spd: 70, spe: 90},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ghost Cellulose"],
		changesFrom: "Arenay",
	},
	arenaydark: {
		num: 50001,
		name: "Arenay-Dark",
		baseSpecies: "Arenay",
		forme: "Dark",
		types: ["Dark"],
		gender: "N",
		baseStats: {hp: 90, atk: 100, def: 60, spa: 100, spd: 70, spe: 70},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dark Cellulose"],
		changesFrom: "Arenay",
	},
	arenayfairy: {
		num: 50001,
		name: "Arenay-Fairy",
		baseSpecies: "Arenay",
		forme: "Fairy",
		types: ["Fairy"],
		gender: "N",
		baseStats: {hp: 90, atk: 70, def: 50, spa: 100, spd: 120, spe: 60},
		abilities: {0: "Splice"},
		heightm: 0.3,
		weightkg: 6.5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Psychic Cellulose"],
		changesFrom: "Arenay",
	},
	Eevee: {
		inherit: true,
		evos: ["Vaporeon", "Jolteon", "Flareon", "Espeon", "Umbreon", "Leafeon", "Glaceon", "Sylveon", "Aeveon", "Chromeon"],
	},
	aeveon: {
		num: 50002,
		name: "Aeveon",
		types: ["Flying"],
		baseStats: {hp: 95, atk: 65, def: 110, spa: 60, spd: 130, spe: 65},
		abilities: {0: "Synchronize", H: "Inner Focus"},
		heightm: 1,
		weightkg: 35,
		color: "Black",
		prevo: "Eevee",
		evoType: "useItem",
		evoItem: "Bio Stone",
		eggGroups: ["Field"],
	},
	chromeon: {
		num: 50003,
		name: "Chromeon",
		types: ["Steel"],
		baseStats: {hp: 95, atk: 65, def: 110, spa: 60, spd: 130, spe: 65},
		abilities: {0: "Synchronize", H: "Inner Focus"},
		heightm: 1,
		weightkg: 98,
		color: "Black",
		prevo: "Eevee",
		evoType: "useItem",
		evoItem: "Mech Stone",
		eggGroups: ["Field"],
	},
	magikarp: {
		inherit: true,
		otherFormes: ["Magikarp-Stacona"],
		formeOrder: ["Magikarp", "Magikarp-Stacona"],
	},
	magikarpstacona: {
		num: 129,
		name: "Magikarp-Stacona",
		baseSpecies: "Magikarp",
		forme: "Stacona",
		types: ["Water", "Electric"],
		baseStats: {hp: 20, atk: 10, def: 55, spa: 15, spd: 20, spe: 80},
		abilities: {0: "Static", 1: "Swift Swim", H: "Lightning Rod"},
		heightm: 0.9,
		weightkg: 10,
		color: "Blue",
		evos: ["Gyarados-Stacona"],
		eggGroups: ["Water 2", "Dragon"],
	},
	gyarados: {
		inherit: true,
		otherFormes: ["Gyarados-Mega", "Gyarados-Stacona"],
		formeOrder: ["Gyarados", "Gyarados-Mega", "Gyarados-Stacona"],
	},
	gyaradosstacona: {
		num: 130,
		name: "Gyarados-Stacona",
		baseSpecies: "Gyarados",
		forme: "Stacona",
		types: ["Water", "Electric"],
		baseStats: {hp: 95, atk: 125, def: 79, spa: 60, spd: 100, spe: 81},
		abilities: {0: "Static", 1: "Intimidate", H: "Lightning Rod"},
		heightm: 6.5,
		weightkg: 235,
		color: "Blue",
		evos: ["Gyarados-Stacona"],
		eggGroups: ["Water 2", "Dragon"],
	},
	lappy: {
		num: 50004,
		name: "Lappy",
		types: ["Water"],
		baseStats: {hp: 100, atk: 65, def: 90, spa: 65, spd: 65, spe: 40},
		abilities: {0: "Swift Swim", 1: "Water Absorb", H: "Hydration"},
		heightm: 2.5,
		weightkg: 220,
		color: "Blue",
		evos: ["Lapras"],
		eggGroups: ["Monster", "Water 1"],
	},
	lapras: {
		inherit: true,
		prevo: "Lappy",
	},
	babee: {
		num: 50005,
		name: "Babee",
		types: ["Bug", "Flying"],
		baseStats: {hp: 25, atk: 25, def: 36, spa: 25, spd: 36, spe: 50},
		abilities: {0: "Honey Gather", H: "Hustle"},
		heightm: 0.3,
		weightkg: 5.5,
		color: "Yellow",
		evos: ["Combee"],
		eggGroups: ["Bug"],
	},
	combee: {
		inherit: true,
		prevo: "Babee",
		evos: ["Vespiquen", "Polleknight"],
	},
	polleknight: {
		num: 50006,
		name: "Polleknight",
		types: ["Bug", "Flying"],
		baseStats: {hp: 75, atk: 125, def: 60, spa: 70, spd: 30, spe: 102},
		abilities: {0: "Shield Dust", 1: "Swarm", H: "Technician"},
		heightm: 0.3,
		weightkg: 5.5,
		color: "Yellow",
		prevo: ["Combee"],
		eggGroups: ["Bug"],
	},
	adorabull: {
		num: 50007,
		name: "Adorabull",
		types: ["Normal"],
		baseStats: {hp: 45, atk: 70, def: 75, spa: 25, spd: 50, spe: 85},
		abilities: {0: "Cute Charm", 1: "Oblivious", H: "Sheer Force"},
		heightm: 1.4,
		weightkg: 88.5,
		color: "Brown",
		evos: ["Miltank", "Tauros"],
		eggGroups: ["Field"],
	},
	miltank: {
		inherit: true,
		prevo: "Adorabull",
	},
	tauros: {
		inherit: true,
		prevo: "Adorabull",
	},
	pincush: {
		num: 50008,
		name: "Pincush",
		types: ["Bug"],
		baseStats: {hp: 55, atk: 90, def: 80, spa: 45, spd: 50, spe: 65},
		abilities: {0: "Hyper Cutter", 1: "Mold Breaker", H: "Moxie"},
		heightm: 1.1,
		weightkg: 34,
		color: "Brown",
		evos: ["Pinsir"],
		eggGroups: ["Bug"],
	},
	pinsir: {
		inherit: true,
		prevo: "Pincush",
		evos: ["Pinterra"],
	},
	pinterra: {
		num: 50009,
		name: "Pinterra",
		types: ["Bug", "Ground"],
		baseStats: {hp: 15, atk: 130, def: 105, spa: 60, spd: 90, spe: 90},
		abilities: {0: "Hyper Cutter", 1: "Mold Breaker", H: "Moxie"},
		heightm: 2.5,
		weightkg: 85,
		color: "Brown",
		prevo: ["Pinsir"],
		eggGroups: ["Bug"],
	},
	sickler: {
		num: 50010,
		name: "Sickler",
		types: ["Bug"],
		baseStats: {hp: 50, atk: 90, def: 50, spa: 45, spd: 60, spe: 85},
		abilities: {0: "Swarm", 1: "Technician", H: "Steadfast"},
		heightm: 1.5,
		weightkg: 56,
		color: "Green",
		evos: ["Scyther"],
		eggGroups: ["Bug"],
	},
	scyther: {
		inherit: true,
		prevo: "Sickler",
	},
	heracute: {
		num: 50011,
		name: "Heracute",
		types: ["Bug"],
		baseStats: {hp: 60, atk: 70, def: 55, spa: 25, spd: 55, spe: 65},
		abilities: {0: "Swarm", 1: "Guts", H: "Moxie"},
		heightm: 0.8,
		weightkg: 26,
		color: "Blue",
		evos: ["Heracross"],
		eggGroups: ["Bug"],
	},
	heracross: {
		inherit: true,
		prevo: "Heracute",
		evos: ["Heraplex"],
	},
	heraplex: {
		num: 50012,
		name: "Heraplex",
		types: ["Bug", "Fighting"],
		baseStats: {hp: 110, atk: 155, def: 90, spa: 45, spd: 100, spe: 90},
		abilities: {0: "Swarm", 1: "Guts", H: "Moxie"},
		heightm: 2.1,
		weightkg: 74,
		color: "Blue",
		prevo: ["Heracross"],
		eggGroups: ["Bug"],
	},
	foongus: {
		inherit: true,
		otherFormes: ["Foongus-Stacona"],
		formeOrder: ["Foongus", "Foongus-Stacona"],
	},
	foongusstacona: {
		num: 590,
		name: "Foonguss-Stacona",
		baseSpecies: "Foonguss",
		forme: "Stacona",
		types: ["Poison", "Psychic"],
		baseStats: {hp: 69, atk: 45, def: 45, spa: 65, spd: 55, spe: 15},
		abilities: {0: "Effect Spore", H: "Regenerator"},
		heightm: 0.2,
		weightkg: 1,
		color: "Blue",
		evos: ["Amoonguss-Stacona"],
		eggGroups: ["Grass"],
	},
	amoonguss: {
		inherit: true,
		otherFormes: ["Amoonguss-Stacona"],
		formeOrder: ["Amoonguss", "Amoonguss-Stacona"],
	},
	amoongussstacona: {
		num: 591,
		name: "Amoonguss-Stacona",
		baseSpecies: "Amoonguss",
		forme: "Stacona",
		types: ["Poison", "Psychic"],
		baseStats: {hp: 115, atk: 65, def: 70, spa: 115, spd: 80, spe: 30},
		abilities: {0: "Effect Spore", H: "Regenerator"},
		heightm: 0.6,
		weightkg: 10.5,
		color: "Blue",
		prevo: ["Foongus-Stacona"],
		eggGroups: ["Grass"],
	},
	nosepass: {
		inherit: true,
		otherFormes: ["Nosepass-Stacona"],
		formeOrder: ["Nosepass", "Nosepass-Stacona"],
	},
	nosepassstacona: {
		num: 299,
		name: "Nosepass-Stacona",
		baseSpecies: "Nosepass",
		forme: "Stacona",
		types: ["Rock", "Electric"],
		baseStats: {hp: 30, atk: 45, def: 115, spa: 65, spd: 90, spe: 30},
		abilities: {0: "Static", H: "Volt Absorb"},
		heightm: 1,
		weightkg: 97,
		color: "Gray",
		evos: ["Probopass-Stacona"],
		eggGroups: ["Mineral"],
	},
	probopass: {
		inherit: true,
		otherFormes: ["Probopass-Stacona"],
		formeOrder: ["Probopass", "Probopass-Stacona"],
	},
	probopassstacona: {
		num: 476,
		name: "Probopass-Stacona",
		baseSpecies: "Probopass",
		forme: "Stacona",
		types: ["Rock", "Electric"],
		baseStats: {hp: 60, atk: 55, def: 115, spa: 105, spd: 150, spe: 40},
		abilities: {0: "Static", H: "Volt Absorb"},
		heightm: 0.6,
		weightkg: 10.5,
		color: "Blue",
		prevo: ["Probopass-Stacona"],
		eggGroups: ["Grass"],
	},
	zangoose: {
		inherit: true,
		otherFormes: ["Zangoose-Stacona"],
		formeOrder: ["Zangoose", "Zangoose-Stacona"],
	},
	zangoosestacona: {
		num: 335,
		name: "Zangoose-Stacona",
		baseSpecies: "Zangoose",
		forme: "Stacona",
		types: ["Normal", "Fairy"],
		baseStats: {hp: 93, atk: 115, def: 50, spa: 50, spd: 70, spe: 80},
		abilities: {0: "Pixilate", H: "Klutz"},
		heightm: 2.7,
		weightkg: 52.5,
		color: "Pink",
		eggGroups: ["Field"],
	},
	seviper: {
		inherit: true,
		otherFormes: ["Seviper-Stacona"],
		formeOrder: ["Seviper", "Seviper-Stacona"],
	},
	seviperstacona: {
		num: 336,
		name: "Seviper-Stacona",
		baseSpecies: "Seviper",
		forme: "Stacona",
		types: ["Poison", "Rock"],
		baseStats: {hp: 73, atk: 110, def: 60, spa: 65, spd: 60, spe: 90},
		abilities: {0: "Stench", H: "Sturdy"},
		heightm: 2.7,
		weightkg: 52.5,
		color: "Black",
		eggGroups: ["Field", "Dragon"],
	},
	zubat: {
		inherit: true,
		otherFormes: ["Zubat-Stacona"],
		formeOrder: ["Zubat", "Zubat-Stacona"],
	},
	zubatstacona: {
		num: 41,
		name: "Zubat-Stacona",
		baseSpecies: "Zubat",
		forme: "Stacona",
		types: ["Dark", "Flying"],
		baseStats: {hp: 40, atk: 45, def: 30, spa: 30, spd: 40, spe: 60},
		abilities: {0: "Volt Absorb", 1: "Galvanize", H: "Infiltrator", S: "Intimidate"},
		heightm: 0.8,
		weightkg: 7.5,
		color: "Black",
		evos: ["Golbat-Stacona"],
		eggGroups: ["Flying"],
	},
	golbat: {
		inherit: true,
		otherFormes: ["Golbat-Stacona"],
		formeOrder: ["Golbat", "Golbat-Stacona"],
	},
	golbatstacona: {
		num: 42,
		name: "Golbat-Stacona",
		baseSpecies: "Golbat",
		forme: "Stacona",
		types: ["Dark", "Flying"],
		baseStats: {hp: 75, atk: 90, def: 70, spa: 65, spd: 55, spe: 100},
		abilities: {0: "Volt Absorb", 1: "Galvanize", H: "Infiltrator", S: "Intimidate"},
		heightm: 1.6,
		weightkg: 55,
		color: "Black",
		evos: ["Crobat-Stacona"],
		prevo: "Zubat-Stacona",
		eggGroups: ["Flying"],
	},
	crobat: {
		inherit: true,
		otherFormes: ["Crobat-Stacona"],
		formeOrder: ["Crobat", "Crobat-Stacona"],
	},
	crobatstacona: {
		num: 169,
		name: "Crobat-Stacona",
		baseSpecies: "Crobat",
		forme: "Stacona",
		types: ["Dark", "Flying"],
		baseStats: {hp: 95, atk: 100, def: 70, spa: 70, spd: 50, spe: 140},
		abilities: {0: "Volt Absorb", 1: "Galvanize", H: "Infiltrator", S: "Intimidate"},
		heightm: 1.8,
		weightkg: 75,
		color: "Black",
		prevo: "Golbat-Stacona",
		eggGroups: ["Flying"],
	},
	wingot: {
		num: 50013,
		name: "Wingot",
		types: ["Steel", "Flying"],
		baseStats: {hp: 40, atk: 55, def: 100, spa: 30, spd: 40, spe: 40},
		abilities: {0: "Keen Eye", 1: "Sturdy", H: "Weak Armor"},
		heightm: 0.9,
		weightkg: 20.5,
		color: "Gray",
		evos: ["Skarmory"],
		eggGroups: ["Flying"],
	},
	skarmory: {
		inherit: true,
		prevo: "Wingot",
		evos: ["Skartillery"],
	},
	skartillery: {
		num: 50014,
		name: "Wingot",
		types: ["Steel", "Flying"],
		baseStats: {hp: 75, atk: 100, def: 160, spa: 45, spd: 75, spe: 80},
		abilities: {0: "Keen Eye", 1: "Sturdy", H: "Weak Armor"},
		heightm: 2.3,
		weightkg: 70.4,
		color: "Gray",
		evos: ["Skarmory"],
		eggGroups: ["Flying"],
	},
	klink: {
		inherit: true,
		otherFormes: ["Klink-Stacona"],
		formeOrder: ["Klink", "Klink-Stacona"],
	},
	klinkstacona: {
		num: 599,
		name: "Klink-Stacona",
		baseSpecies: "Klink",
		forme: "Stacona",
		types: ["Steel", "Fire"],
		baseStats: {hp: 40, atk: 55, def: 50, spa: 55, spd: 60, spe: 50},
		abilities: {0: "Steampunk", 1: "Motor Drive", H: "White Smoke"},
		heightm: 0.3,
		weightkg: 21,
		color: "Gray",
		evos: ["Klang-Stacona"],
		eggGroups: ["Mineral"],
	},
	klang: {
		inherit: true,
		otherFormes: ["Klang-Stacona"],
		formeOrder: ["Klang", "Klang-Stacona"],
	},
	klangstacona: {
		num: 600,
		name: "Klang-Stacona",
		baseSpecies: "Klang",
		forme: "Stacona",
		types: ["Steel", "Fire"],
		baseStats: {hp: 60, atk: 80, def: 75, spa: 80, spd: 85, spe: 70},
		abilities: {0: "Steampunk", 1: "Motor Drive", H: "White Smoke"},
		heightm: 0.6,
		weightkg: 51,
		color: "Gray",
		evos: ["Klinklang-Stacona"],
		prevo: "Klink-Stacona",
		eggGroups: ["Mineral"],
	},
	klinklang: {
		inherit: true,
		otherFormes: ["Klinklang-Stacona"],
		formeOrder: ["Klinklang", "Klinklang-Stacona"],
	},
	klinklangstacona: {
		num: 600,
		name: "Klinklang-Stacona",
		baseSpecies: "Klinklang",
		forme: "Stacona",
		types: ["Steel", "Fire"],
		baseStats: {hp: 60, atk: 100, def: 95, spa: 90, spd: 85, spe: 110},
		abilities: {0: "Steampunk", 1: "Motor Drive", H: "White Smoke"},
		heightm: 0.6,
		weightkg: 81,
		color: "Gray",
		prevo: "Klang-Stacona",
		eggGroups: ["Mineral"],
	},
	dunsparce: {
		inherit: true,
		otherFormes: ["Dunsparce-Stacona"],
		formeOrder: ["Dunsparce", "Dunsparce-Stacona"],
	},
	dunsparcestacona: {
		num: 206,
		name: "Dunsparce-Stacona",
		baseSpecies: "Dunsparce",
		forme: "Stacona",
		types: ["Dark"],
		baseStats: {hp: 90, atk: 80, def: 70 , spa: 55, spd: 65, spe: 55},
		abilities: {0: "Intimidate", 1: "Moxie", H: "Justified"},
		heightm: 1.5,
		weightkg: 14,
		color: "Purple",
		evos: ["Dundrake"],
		eggGroups: ["Field"],
	},
	dundrake: {
		num: 50015,
		name: "Drundrake",
		types: ["Dark", "Dragon"],
		baseStats: {hp: 110, atk: 110, def: 80, spa: 70, spd: 60, spe: 85},
		abilities: {0: "Intimidate", 1: "Moxie", H: "Justified"},
		heightm: 4.2,
		weightkg: 72,
		color: "Purple",
		prevo: ["Dunsparce-Stacona"],
		eggGroups: ["Field"],
	},
	lotad: {
		inherit: true,
		otherFormes: ["Lotad-Stacona"],
		formeOrder: ["Lotad", "Lotad-Stacona"],
	},
	lotadstacona: {
		num: 270,
		name: "Lotad-Stacona",
		baseSpecies: "Lotad",
		forme: "Stacona",
		types: ["Grass", "Water"],
		baseStats: {hp: 40, atk: 50, def: 30, spa: 35, spd: 35, spe: 30},
		abilities: {0: "Submerged", 1: "Thick Fat", H: "Drizzle"},
		heightm: 0.5,
		weightkg: 2.6,
		color: "Blue",
		evos: ["Lombre-Stacona"],
		eggGroups: ["Water 1", "Grass"],
	},
	lombre: {
		inherit: true,
		otherFormes: ["Lombre-Stacona"],
		formeOrder: ["Lombre", "Lombre-Stacona"],
	},
	lombrestacona: {
		num: 271,
		name: "Lombre-Stacona",
		baseSpecies: "Lombre",
		forme: "Stacona",
		types: ["Grass", "Water"],
		baseStats: {hp: 60, atk: 70, def: 50, spa: 60, spd: 60, spe: 50},
		abilities: {0: "Submerged", 1: "Thick Fat", H: "Drizzle"},
		heightm: 1.2,
		weightkg: 32.5,
		color: "Blue",
		evos: ["Ludicolo-Stacona"],
		prevo: "Lotad-Stacona",
		eggGroups: ["Water 1", "Grass"],
	},
	ludicolo: {
		inherit: true,
		otherFormes: ["Ludicolo-Stacona"],
		formeOrder: ["Ludicolo", "Ludicolo-Stacona"],
	},
	ludicolostacona: {
		num: 272,
		name: "Ludicolo-Stacona",
		baseSpecies: "Ludicolo",
		forme: "Stacona",
		types: ["Grass", "Water"],
		baseStats: {hp: 80, atk: 100, def: 70, spa: 80, spd: 80, spe: 70},
		abilities: {0: "Submerged", 1: "Thick Fat", H: "Drizzle"},
		heightm: 1.5,
		weightkg: 55,
		color: "Blue",
		prevo: "Lombre-Stacona",
		eggGroups: ["Water 1", "Grass"],
	},
	solosis: {
		inherit: true,
		otherFormes: ["Solosis-Stacona"],
		formeOrder: ["Solosis", "Solosis-Stacona"],
	},
	solosisstacona: {
		num: 577,
		name: "Solosis-Stacona",
		baseSpecies: "Solosis",
		forme: "Stacona",
		types: ["Grass", "Psychic"],
		baseStats: {hp: 45, atk: 30, def: 40, spa: 115, spd: 50, spe: 30},
		abilities: {0: "Chlorophyll", 1: "Magic Guard", H: "Psychic Surge"},
		heightm: 0.3,
		weightkg: 1,
		color: "Green",
		evos: ["Duosion-Stacona"],
		eggGroups: ["Amorphous"],
	},
	duosion: {
		inherit: true,
		otherFormes: ["Duosion-Stacona"],
		formeOrder: ["Duosion", "Duosion-Stacona"],
	},
	duosionstacona: {
		num: 578,
		name: "Duosion-Stacona",
		baseSpecies: "Duosion",
		forme: "Stacona",
		types: ["Grass", "Psychic"],
		baseStats: {hp: 65, atk: 40, def: 50, spa: 135, spd: 60, spe: 40},
		abilities: {0: "Chlorophyll", 1: "Magic Guard", H: "Psychic Surge"},
		heightm: 0.6,
		weightkg: 8,
		color: "Green",
		evos: ["Reuniclus-Stacona"],
		prevo: "Solosis-Stacona",
		eggGroups: ["Amorphous"],
	},
	reuniclus: {
		inherit: true,
		otherFormes: ["Reuniclus-Stacona"],
		formeOrder: ["Reuniclus", "Reuniclus-Stacona"],
	},
	reuniclusstacona: {
		num: 579,
		name: "Reuniclus-Stacona",
		baseSpecies: "Reuniclus",
		forme: "Stacona",
		types: ["Grass", "Psychic"],
		baseStats: {hp: 115, atk: 65, def: 70, spa: 140, spd: 85, spe: 45},
		abilities: {0: "Chlorophyll", 1: "Magic Guard", H: "Psychic Surge"},
		heightm: 1,
		weightkg: 20.1,
		color: "Green",
		prevo: "Duosion-Stacona",
		eggGroups: ["Amorphous"],
	},
	krabby: {
		inherit: true,
		otherFormes: ["Krabby-Stacona"],
		formeOrder: ["Krabby", "Krabby-Stacona"],
	},
	krabbystacona: {
		num: 98,
		name: "Krabby-Stacona",
		baseSpecies: "Krabby",
		forme: "Stacona",
		types: ["Water", "Steel"],
		baseStats: {hp: 30, atk: 105, def: 90, spa: 25, spd: 25, spe: 50},
		abilities: {0: "Hyper Cutter", 1: "Shell Armor", H: "Steelworker"},
		heightm: 0.4,
		weightkg: 6.5,
		color: "Blue",
		evos: ["Kingler-Stacona"],
		eggGroups: ["Water 3"],
	},
	kingler: {
		inherit: true,
		otherFormes: ["Kingler-Stacona"],
		formeOrder: ["Kingler", "Kingler-Stacona"],
	},
	kinglerstacona: {
		num: 99,
		name: "Kingler-Stacona",
		baseSpecies: "Kingler",
		forme: "Stacona",
		types: ["Water", "Steel"],
		baseStats: {hp: 55, atk: 130, def: 115, spa: 50, spd: 50, spe: 75},
		abilities: {0: "Hyper Cutter", 1: "Shell Armor", H: "Steelworker"},
		heightm: 1.3,
		weightkg: 60,
		color: "Blue",
		prevo: "Krabby-Stacona",
		eggGroups: ["Water 3"],
	},
	sandile: {
		inherit: true,
		otherFormes: ["Sandile-Stacona"],
		formeOrder: ["Sandile", "Sandile-Stacona"],
	},
	sandilestacona: {
		num: 551,
		name: "Sandile-Stacona",
		baseSpecies: "Sandile",
		forme: "Stacona",
		types: ["Water", "Dark"],
		baseStats: {hp: 50, atk: 72, def: 35, spa: 35, spd: 35, spe: 65},
		abilities: {0: "Intimidate", 1: "Water Absorb", H: "Moxie"},
		heightm: 0.7,
		weightkg: 15.2,
		color: "Blue",
		evos: ["Krokorok-Stacona"],
		eggGroups: ["Field"],
	},
	krokorok: {
		inherit: true,
		otherFormes: ["Krokorok-Stacona"],
		formeOrder: ["Krokorok", "Krokorok-Stacona"],
	},
	krokorokstacona: {
		num: 552,
		name: "Duosion-Stacona",
		baseSpecies: "Duosion",
		forme: "Stacona",
		types: ["Water", "Dark"],
		baseStats: {hp: 60, atk: 82, def: 45, spa: 45, spd: 45, spe: 74},
		abilities: {0: "Intimidate", 1: "Water Absorb", H: "Moxie"},
		heightm: 1,
		weightkg: 33.4,
		color: "Blue",
		evos: ["Krookodile-Stacona"],
		prevo: "Sandile-Stacona",
		eggGroups: ["Field"],
	},
	krookodile: {
		inherit: true,
		otherFormes: ["Krookodile-Stacona"],
		formeOrder: ["Krookodile", "Krookodile-Stacona"],
	},
	krookodilestacona: {
		num: 553,
		name: "Krookodile-Stacona",
		baseSpecies: "Krookodile",
		forme: "Stacona",
		types: ["Water", "Dark"],
		baseStats: {hp: 115, atk: 65, def: 70, spa: 140, spd: 85, spe: 45},
		abilities: {0: "Intimidate", 1: "Water Absorb", H: "Moxie"},
		heightm: 1.5,
		weightkg: 96.3,
		color: "Blue",
		prevo: "Krokorok-Stacona",
		eggGroups: ["Field"],
	},
	kanga: {
		num: 50017,
		name: "Kanga",
		types: ["Normal"],
		baseStats: {hp: 80, atk: 75, def: 90, spa: 30, spd: 65, spe: 70},
		abilities: {0: "Early Bird", 1: "Scrappy", H: "Inner Focus"},
		heightm: 1.1,
		weightkg: 32,
		color: "Brown",
		evos: ["Kangaskhan"],
		eggGroups: ["Monster"],
	},
	kangaskhan: {
		inherit: true,
		prevo: "Kanga",
	},
	pichu: {
		inherit: true,
		otherFormes: ["Pichu-Spiky-eared", "Pichu-Mech"],
		formeOrder: ["Pichu", "Pichu-Spiky-eared", "Pichu-Mech"],
	},
	pichumech: {
		num: 172,
		name: "Pichu-Mech",
		baseSpecies: "Pichu",
		forme: "Stacona",
		types: ["Steel", "Electric"],
		baseStats: {hp: 20, atk: 40, def: 35, spa: 45, spd: 25, spe: 40},
		abilities: {0: "Motor Drive", 1: "Download", H: "Volt Absorb"},
		heightm: 0.3,
		weightkg: 2,
		color: "Gray",
		evos: ["Pikachu-Mech"],
		eggGroups: ["Undiscovered"],
	},
	pikachu: {
		inherit: true,
		otherFormes: ["Pikachu-Cosplay", "Pikachu-Rock-Star", "Pikachu-Belle", "Pikachu-Pop-Star", "Pikachu-PhD", "Pikachu-Libre", "Pikachu-Original", "Pikachu-Hoenn", "Pikachu-Sinnoh", "Pikachu-Unova", "Pikachu-Kalos", "Pikachu-Alola", "Pikachu-Partner", "Pikachu-Starter", "Pikachu-World", "Pikachu-Mech"],
		formeOrder: [
			"Pikachu",
			"Pikachu-Original", "Pikachu-Hoenn", "Pikachu-Sinnoh", "Pikachu-Unova", "Pikachu-Kalos", "Pikachu-Alola", "Pikachu-Partner", // Gen 7 formes
			"Pikachu-Starter", // forme 8 in LGPE
			"Pikachu-World", // Gen 8 forme (indexed as Pikachu 9 in datamine)
			"Pikachu-Rock-Star", "Pikachu-Belle", "Pikachu-Pop-Star", "Pikachu-PhD", "Pikachu-Libre", "Pikachu-Cosplay", // formes 1-6 from Gen 6
			"Pikachu-Mech"
		]
	},
	pikachumech: {
		num: 25,
		name: "Pikachu-Mech",
		baseSpecies: "Pikachu",
		forme: "Mech",
		types: ["Steel", "Electric"],
		baseStats: {hp: 40, atk: 60, def: 50, spa: 70, spd: 50, spe: 60},
		abilities: {0: "Motor Drive", 1: "Download", H: "Volt Absorb"},
		heightm: 0.4,
		weightkg: 6,
		color: "Gray",
		evos: ["Raichu-Mech"],
		prevo: "Pichu-Mech",
		eggGroups: ["Field"],
	},
	raichu: {
		inherit: true,
		otherFormes: ["Raichu-Alola", "Raichu-Mech"],
		formeOrder: ["Raichu", "Raichu-Alola", "Raichu-Mech"],
	},
	raichumech: {
		num: 26,
		name: "Raichu-Mech",
		baseSpecies: "Raichu",
		forme: "Mech",
		types: ["Steel", "Electric"],
		baseStats: {hp: 60, atk: 80, def: 95, spa: 90, spd: 70, spe: 80},
		abilities: {0: "Intimidate", 1: "Water Absorb", H: "Moxie"},
		heightm: 0.7,
		weightkg: 21,
		color: "Blue",
		prevo: "Pikachu-Mech",
		eggGroups: ["Field"],
	},
	marbolt: {
		num: 50018,
		name: "Marbolt",
		types: ["Electric"],
		baseStats: {hp: 30, atk: 25, def: 40, spa: 45, spd: 45, spe: 90},
		abilities: {0: "Soundproof", 1: "Static", H: "Aftermath"},
		heightm: 0.2,
		weightkg: 2.4,
		color: "Red",
		evos: ["Voltorb"],
		eggGroups: ["Mineral"],
	},
	voltorb: {
		inherit: true,
		prevo: "Marbolt",
	},
	mareep: {
		inherit: true,
		otherFormes: ["Mareep-Stacona"],
		formeOrder: ["Mareep", "Mareep-Stacona"],
	},
	mareepstacona: {
		num: 179,
		name: "Mareep-Stacona",
		baseSpecies: "Mareep",
		forme: "Stacona",
		types: ["Ice", "Electric"],
		baseStats: {hp: 55, atk: 40, def: 40, spa: 65, spd: 45, spe: 35},
		abilities: {0: "Ice Body", 1: "Snow Warning", H: "Lightning Rod"},
		heightm: 0.6,
		weightkg: 7.8,
		color: "White",
		evos: ["Flaaffy-Stacona"],
		eggGroups: ["Monster", "Field"],
	},
	flaaffy: {
		inherit: true,
		otherFormes: ["Flaaffy-Stacona"],
		formeOrder: ["Flaaffy", "Flaaffy-Stacona"],
	},
	flaaffystacona: {
		num: 180,
		name: "Flaaffy-Stacona",
		baseSpecies: "Flaaffy",
		forme: "Stacona",
		types: ["Ice", "Electric"],
		baseStats: {hp: 70, atk: 55, def: 55, spa: 80, spd: 60, spe: 45},
		abilities: {0: "Ice Body", 1: "Snow Warning", H: "Lightning Rod"},
		heightm: 0.8,
		weightkg: 13.3,
		color: "White",
		evos: ["Ampharos-Stacona"],
		prevo: "Mareep-Stacona",
		eggGroups: ["Monster", "Field"],
	},
	ampharos: {
		inherit: true,
		otherFormes: ["Ampharos-Stacona"],
		otherFormes: ["Ampharos-Mega", "Ampharos-Stacona"],
		formeOrder: ["Ampharos", "Ampharos-Mega", "Ampharos-Stacona"],
	},
	ampharosstacona: {
		num: 181,
		name: "Ampharos-Stacona",
		baseSpecies: "Ampharos",
		forme: "Stacona",
		types: ["Ice", "Electric"],
		baseStats: {hp: 90, atk: 75, def: 85, spa: 115, spd: 90, spe: 55},
		abilities: {0: "Ice Body", 1: "Snow Warning", H: "Lightning Rod"},
		heightm: 1.4,
		weightkg: 61.5,
		color: "White",
		prevo: "Flaaffy-Stacona",
		eggGroups: ["Monster", "Field"],
	},
	sentret: {
		inherit: true,
		otherFormes: ["Sentret-Stacona"],
		formeOrder: ["Sentret", "Sentret-Stacona"],
	},
	sentretstacona: {
		num: 161,
		name: "Sentret-Stacona",
		baseSpecies: "Sentret",
		forme: "Stacona",
		types: ["Normal", "Fairy"],
		baseStats: {hp: 34, atk: 35, def: 20, spa: 46, spd: 45, spe: 35},
		abilities: {0: "Cute Charm", 1: "Frisk", H: "Magic Guard"},
		heightm: 0.8,
		weightkg: 6,
		color: "Pink",
		evos: ["Furret-Stacona"],
		eggGroups: ["Field"],
	},
	furret: {
		inherit: true,
		otherFormes: ["Furret-Stacona"],
		formeOrder: ["Furret", "Furret-Stacona"],
	},
	furretstacona: {
		num: 168,
		name: "Furret-Stacona",
		baseSpecies: "Furret",
		forme: "Stacona",
		types: ["Normal", "Fairy"],
		baseStats: {hp: 74, atk: 45, def: 45, spa: 96, spd: 75, spe: 90},
		abilities: {0: "Cute Charm", 1: "Frisk", H: "Magic Guard"},
		heightm: 1.8,
		weightkg: 32.5,
		color: "Pink",
		prevo: "Sentret-Stacona",
		eggGroups: ["Field"],
	},
	spinarak: {
		inherit: true,
		otherFormes: ["Spinarak-Stacona"],
		formeOrder: ["Spinarak", "Spinarak-Stacona"],
	},
	spinarakstacona: {
		num: 167,
		name: "Spinarak-Stacona",
		baseSpecies: "Spinarak",
		forme: "Stacona",
		types: ["Bug", "Dark"],
		baseStats: {hp: 30, atk: 50, def: 60, spa: 50, spd: 40, spe: 40},
		abilities: {0: "Prankster", 1: "Stall", H: "Intimidate"},
		heightm: 0.4,
		weightkg: 6.5,
		color: "Black",
		evos: ["Ariados-Stacona"],
		eggGroups: ["Bug"],
	},
	ariados: {
		inherit: true,
		otherFormes: ["Ariados-Stacona"],
		formeOrder: ["Ariados", "Ariados-Stacona"],
	},
	ariadosstacona: {
		num: 168,
		name: "Ariados-Stacona",
		baseSpecies: "Ariados",
		forme: "Stacona",
		types: ["Bug", "Dark"],
		baseStats: {hp: 40, atk: 95, def: 90, spa: 75, spd: 40, spe: 60},
		abilities: {0: "Prankster", 1: "Stall", H: "Intimidate"},
		heightm: 1.1,
		weightkg: 33.5,
		color: "Black",
		prevo: "Spinarak-Stacona",
		eggGroups: ["Bug"],
	},
	shuckle: {
		inherit: true,
		evos: ["Shuckavern"],
	},
	shuckavern: {
		num: 50018,
		name: "Shuckavern",
		types: ["Bug", "Rock"],
		baseStats: {hp: 25, atk: 15, def: 250, spa: 15, spd: 240, spe: 10},
		abilities: {0: "Sturdy", 1: "Cheek Pouch", H: "Contrary"},
		heightm: 1.1,
		weightkg: 51.4,
		color: "Yellow",
		prevo: ["Shuckle"],
		eggGroups: ["Bug"],
	},
	stantler: {
		inherit: true,
		evos: ["Gigantler"],
	},
	gigantler: {
		num: 50019,
		name: "Gigantler",
		types: ["Normal"],
		baseStats: {hp: 100, atk: 120, def: 90, spa: 90, spd: 75, spe: 100},
		abilities: {0: "Intimidate", 1: "Frisk", H: "Sap Sipper"},
		heightm: 2.3,
		weightkg: 94.1,
		color: "Brown",
		prevo: ["Stantler"],
		eggGroups: ["Bug"],
	},
	elekid: {
		inherit: true,
		otherFormes: ["Elekid-Stacona"],
		formeOrder: ["Elekid", "Elekid-Stacona"],
	},
	elekidstacona: {
		num: 239,
		name: "Elekid-Stacona",
		baseSpecies: "Elekid",
		forme: "Stacona",
		types: ["Electric", "Fighting"],
		baseStats: {hp: 45, atk: 85, def: 37, spa: 63, spd: 55, spe: 65},
		abilities: {0: "Flash Fist", 1: "Iron Fist", H: "Electric Surge"},
		heightm: 0.6,
		weightkg: 23.5,
		color: "Brown",
		evos: ["Electabuzz-Stacona"],
		eggGroups: ["Undiscovered"],
	},
	electabuzz: {
		inherit: true,
		otherFormes: ["Electabuzz-Stacona"],
		formeOrder: ["Electabuzz", "Electabuzz-Stacona"],
	},
	electabuzzstacona: {
		num: 125,
		name: "Electabuzz-Stacona",
		baseSpecies: "Electabuzz",
		forme: "Stacona",
		types: ["Electric", "Fighting"],
		baseStats: {hp: 65, atk: 103, def: 57, spa: 95, spd: 85, spe: 75},
		abilities: {0: "Flash Fist", 1: "Iron Fist", H: "Electric Surge"},
		heightm: 1.1,
		weightkg: 30,
		color: "Brown",
		evos: ["Electivire-Stacona"],
		prevo: "Elekid-Stacona",
		eggGroups: ["Human-Like"],
	},
	electivire: {
		inherit: true,
		otherFormes: ["Electivire-Stacona"],
		formeOrder: ["Electivire", "Electivire-Stacona"],
	},
	electivirestacona: {
		num: 466,
		name: "Electivire-Stacona",
		baseSpecies: "Electivire",
		forme: "Stacona",
		types: ["Electric", "Fighting"],
		baseStats: {hp: 75, atk: 133, def: 67, spa: 95, spd: 85, spe: 85},
		abilities: {0: "Flash Fist", 1: "Iron Fist", H: "Electric Surge"},
		heightm: 1.8,
		weightkg: 138.6,
		color: "Brown",
		prevo: "Electabuzz-Stacona",
		eggGroups: ["Human-Like"],
	},
	ralts: {
		inherit: true,
		otherFormes: ["Ralts-Stacona"],
		formeOrder: ["Ralts", "Ralts-Stacona"],
	},
	raltsstacona: {
		num: 280,
		name: "Ralts-Stacona",
		baseSpecies: "Ralts",
		forme: "Stacona",
		types: ["Psychic", "Ground"],
		baseStats: {hp: 25, atk: 25, def: 45, spa: 28, spd: 35, spe: 25},
		abilities: {0: "Cursed Body", 1: "Sychronize", H: "No Guard"},
		heightm: 0.4,
		weightkg: 6.6,
		color: "Black",
		evos: ["Kirlia-Stacona"],
		eggGroups: ["Human-Like", "Amorphous"],
	},
	kirlia: {
		inherit: true,
		otherFormes: ["Kirlia-Stacona"],
		formeOrder: ["Kirlia", "Kirlia-Stacona"],
	},
	kirliastacona: {
		num: 281,
		name: "Kirlia-Stacona",
		baseSpecies: "Kirlia",
		forme: "Stacona",
		types: ["Psychic", "Ground"],
		baseStats: {hp: 55, atk: 35, def: 50, spa: 38, spd: 65, spe: 35},
		abilities: {0: "Cursed Body", 1: "Sychronize", H: "No Guard"},
		heightm: 0.8,
		weightkg: 20.2,
		color: "Black",
		evos: ["Gardevoir-Stacona", "Gallade-Stacona"],
		prevo: "Ralts-Stacona",
		eggGroups: ["Human-Like", "Amorphous"],
	},
	gardevoir: {
		inherit: true,
		otherFormes: ["Gardevoir-Mega", "Gardevoir-Stacona"],
		formeOrder: ["Gardevoir", "Gardevoir-Mega", "Gardevoir-Stacona"],
	},
	gardevoirstacona: {
		num: 282,
		name: "Gardevoir-Stacona",
		baseSpecies: "Gardevoir",
		forme: "Stacona",
		types: ["Psychic", "Ground"],
		baseStats: {hp: 68, atk: 55, def: 80, spa: 135, spd: 115, spe: 65},
		abilities: {0: "Levitate", 1: "Cursed Body", H: "Magic Bounce"},
		heightm: 1.6,
		weightkg: 48.4,
		color: "Black",
		prevo: "Kirlia-Stacona",
		eggGroups: ["Human-Like", "Amorphous"],
	},
	gallade: {
		inherit: true,
		otherFormes: ["Gallade-Mega", "Gallade-Stacona"],
		formeOrder: ["Gallade", "Gallade-Mega", "Gallade-Stacona"],
	},
	galladestacona: {
		num: 475,
		name: "Gallade-Stacona",
		baseSpecies: "Gallade",
		forme: "Stacona",
		types: ["Fighting", "Ground"],
		baseStats: {hp: 68, atk: 135, def: 80, spa: 55, spd: 115, spe: 65},
		abilities: {0: "Intimidate", 1: "Inner Focus", H: "No Guard"},
		heightm: 1.6,
		weightkg: 52,
		color: "Black",
		prevo: "Kirlia-Stacona",
		eggGroups: ["Human-Like", "Amorphous"],
	},
	sabling: {
		num: 50020,
		name: "Sabling",
		types: ["Dark", "Ghost"],
		baseStats: {hp: 40, atk: 55, def: 55, spa: 55, spd: 55, spe: 40},
		abilities: {0: "Keen Eye", 1: "Stall", H: "Prankster"},
		heightm: 0.3,
		weightkg: 6,
		color: "Purple",
		evos: ["Sableye"],
		eggGroups: ["Human-Like"],
	},
	sableye: {
		inherit: true,
		prevo: "Sabling",
		evos: ["Sabsidian"],
	},
	sabsidian: {
		num: 50021,
		name: "Sabsidian",
		types: ["Dark", "Ghost"],
		baseStats: {hp: 70, atk: 85, def: 110, spa: 70, spd: 75, spe: 55},
		abilities: {0: "Magic Bounce", 1: "Stall", H: "Prankster"},
		heightm: 2.4,
		weightkg: 81,
		color: "Purple",
		prevo: ["Sableye"],
		eggGroups: ["Human-Like"],
	},
	numel: {
		inherit: true,
		otherFormes: ["Numel-Stacona"],
		formeOrder: ["Numel", "Numel-Stacona"],
	},
	numelstacona: {
		num: 322,
		name: "Numel-Stacona",
		baseSpecies: "Numel",
		forme: "Stacona",
		types: ["Ice", "Ground"],
		baseStats: {hp: 60, atk: 60, def: 40, spa: 65, spd: 45, spe: 35},
		abilities: {0: "Snow Warning", 1: "Sand Stream", H: "Thick Fat"},
		heightm: 0.7,
		weightkg: 24,
		color: "Blue",
		evos: ["Camerupt-Stacona"],
		eggGroups: ["Field"],
	},
	camerupt: {
		inherit: true,
		otherFormes: ["Camerupt-Stacona"],
		formeOrder: ["Camerupt", "Camerupt-Stacona"],
	},
	cameruptstacona: {
		num: 323,
		name: "Camerupt-Stacona",
		baseSpecies: "Camerupt",
		forme: "Stacona",
		types: ["Ice", "Ground"],
		baseStats: {hp: 70, atk: 100, def: 70, spa: 105, spd: 75, spe: 40},
		abilities: {0: "Snow Warning", 1: "Sand Stream", H: "Thick Fat"},
		heightm: 1.9,
		weightkg: 220,
		color: "Blue",
		prevo: "Numel-Stacona",
		eggGroups: ["Field"],
	},
	trapich: {
		inherit: true,
		otherFormes: ["Trapinch-Stacona"],
		formeOrder: ["Trapinch", "Trapinch-Stacona"],
	},
	trapinchstacona: {
		num: 328,
		name: "Trapinch-Stacona",
		baseSpecies: "Trapinch",
		forme: "Stacona",
		types: ["Poison", "Ground"],
		baseStats: {hp: 55, atk: 100, def: 45, spa: 45, spd: 45, spe: 10},
		abilities: {0: "Poison Point", 1: "Stench", H: "Moxie"},
		heightm: 0.7,
		weightkg: 15,
		color: "Brown",
		evos: ["Vibrava-Stacona"],
		eggGroups: ["Bug", "Dragon"],
	},
	vibrava: {
		inherit: true,
		otherFormes: ["Vibrava-Stacona"],
		formeOrder: ["Vibrava", "Vibrava-Stacona"],
	},
	vibravastacona: {
		num: 329,
		name: "Vibrava-Stacona",
		baseSpecies: "Vibrava",
		forme: "Stacona",
		types: ["Poison", "Dragon"],
		baseStats: {hp: 60, atk: 70, def: 50, spa: 50, spd: 50, spe: 70},
		abilities: {0: "Poison Point", 1: "Stench", H: "Moxie"},
		heightm: 1.1,
		weightkg: 15.3,
		color: "Purple",
		evos: ["Flygon-Stacona"],
		prevo: "Trapinch-Stacona",
		eggGroups: ["Bug", "Dragon"],
	},
	flygon: {
		inherit: true,
		otherFormes: ["Flygon-Stacona"],
		formeOrder: ["Flygon", "Flygon-Stacona"],
	},
	flygonstacona: {
		num: 329,
		name: "Flygon-Stacona",
		baseSpecies: "Flygon",
		forme: "Stacona",
		types: ["Poison", "Dragon"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 80, spd: 80, spe: 100},
		abilities: {0: "Poison Point", 1: "Stench", H: "Moxie"},
		heightm: 2,
		weightkg: 82,
		color: "Purple",
		prevo: "Vibrava-Stacona",
		eggGroups: ["Bug", "Dragon"],
	},
	castform: {
		inherit: true,
		otherFormes: ["Castform-Sunny", "Castform-Rainy", "Castform-Snowy", "Castform-Sandy"],
		formeOrder: ["Castform", "Castform-Sunny", "Castform-Rainy", "Castform-Snowy", "Castform-Sandy"],
	},
	castformsandy: {
		num: 351,
		name: "Castform-Sandy",
		baseSpecies: "Castform",
		forme: "Sandy",
		types: ["Rock"],
		baseStats: {hp: 70, atk: 70, def: 70, spa: 70, spd: 70, spe: 70},
		abilities: {0: "Forecast"},
		heightm: 0.3,
		weightkg: 0.8,
		color: "Brown",
		eggGroups: ["Fairy", "Amorphous"],
		requiredAbility: "Forecast",
		battleOnly: "Castform",
	},
	troppy: {
		num: 50022,
		name: "Troppy",
		types: ["Grass"],
		baseStats: {hp: 75, atk: 58, def: 74, spa: 65, spd: 85, spe: 45},
		abilities: {0: "Chlorophyll", 1: "Solar Power", H: "Harvest"},
		heightm: 0.8,
		weightkg: 30,
		color: "Green",
		evos: ["Tropius"],
		eggGroups: ["Monster", "Grass"],
	},
	tropius: {
		inherit: true,
		prevo: "Troppy",
	},
	absol: {
		inherit: true,
		otherFormes: ["Absol-Mega", "Absol-Stacona"],
		formeOrder: ["Absol", "Absol-Mega", "Absol-Stacona"],
	},
	absolstacona: {
		num: 359,
		name: "Absol-Stacona",
		baseSpecies: "Absol",
		forme: "Stacona",
		types: ["Dark", "Steel"],
		baseStats: {hp: 65, atk: 130, def: 100, spa: 45, spd: 50, spe: 75},
		abilities: {0: "Clear Body", 1: "Mold Breaker", H: "Justified"},
		heightm: 1.2,
		weightkg: 47,
		color: "Gray",
		evos: ["Absaster"],
		eggGroups: ["Field"],
	},
	absaster: {
		num: 50023,
		name: "Absaster",
		types: ["Dark", "Steel"],
		baseStats: {hp: 75, atk: 135, def: 105, spa: 80, spd: 65, spe: 85},
		abilities: {0: "Clear Body", 1: "Mold Breaker", H: "Justified"},
		heightm: 1.4,
		weightkg: 76,
		color: "Gray",
		prevo: ["Absol-Stacona"],
		eggGroups: ["Field"],
	},
	drifblim: {
		inherit: true,
		evos: ["Drifship"],
	},
	drifship: {
		num: 50024,
		name: "Drifship",
		types: ["Ghost", "Flying"],
		baseStats: {hp: 180, atk: 90, def: 64, spa: 100, spd: 64, spe: 90},
		abilities: {0: "Aftermath", 1: "Steelworker", H: "Flare Boost"},
		heightm: 100.2,
		weightkg: 460,
		color: "Gray",
		prevo: ["Drifblim"],
		eggGroups: ["Amorphous"],
	},
	rotom: {
		inherit: true,
		otherFormes: ["Rotom-Heat", "Rotom-Wash", "Rotom-Frost", "Rotom-Fan", "Rotom-Mow", "Rotom-Camera"],
		formeOrder: ["Rotom", "Rotom-Heat", "Rotom-Wash", "Rotom-Frost", "Rotom-Fan", "Rotom-Mow", "Rotom-Camera"],
	},
	rotomcamera: {
		num: 479,
		name: "Rotom-Camera",
		baseSpecies: "Rotom",
		forme: "Camera",
		types: ["Electric", "Steel"],
		gender: "N",
		baseStats: {hp: 50, atk: 65, def: 107, spa: 105, spd: 107, spe: 86},
		abilities: {0: "Levitate"},
		heightm: 0.3,
		weightkg: 0.3,
		color: "Gray",
		eggGroups: ["Amorphous"],
		changesFrom: "Rotom",
	},
	blitzle: {
		inherit: true,
		otherFormes: ["Blitzle-Stacona"],
		formeOrder: ["Blitzle", "Blitzle-Stacona"],
	},
	blitzlestacona: {
		num: 522,
		name: "Blitzle-Stacona",
		baseSpecies: "Blitzle",
		forme: "Stacona",
		types: ["Electric", "Fire"],
		baseStats: {hp: 45, atk: 60, def: 32, spa: 50, spd: 32, spe: 76},
		abilities: {0: "Lightning Rod", 1: "Flash Fire", H: "Static"},
		heightm: 0.8,
		weightkg: 29.8,
		color: "Red",
		evos: ["Zebstrika-Stacona"],
		eggGroups: ["Field"],
	},
	zebstrika: {
		inherit: true,
		otherFormes: ["Zebstrika-Stacona"],
		formeOrder: ["Zebstrika", "Zebstrika-Stacona"],
	},
	zebstrikastacona: {
		num: 523,
		name: "Zebstrika-Stacona",
		baseSpecies: "Zebstrika",
		forme: "Stacona",
		types: ["Electric", "Fire"],
		baseStats: {hp: 75, atk: 100, def: 63, spa: 80, spd: 63, spe: 116},
		abilities: {0: "Lightning Rod", 1: "Flash Fire", H: "Static"},
		heightm: 1.6,
		weightkg: 79.5,
		color: "Red",
		prevo: "Blitzle-Stacona",
		eggGroups: ["Field"],
	},
	alumidon: {
		num: 50025,
		name: "Alumidon",
		types: ["Steel", "Dragon"],
		baseStats: {hp: 50, atk: 65, def: 70, spa: 55, spd: 50, spe: 50},
		abilities: {0: "Light Metal", 1: "Heavy Metal", H: "Stalwart"},
		heightm: 0.5,
		weightkg: 16,
		color: "Green",
		evos: ["Duraludon"],
		eggGroups: ["Mineral", "Dragon"],
	},
	duraludon: {
		inherit: true,
		prevo: "Alumidon",
	},
	swablu: {
		inherit: true,
		otherFormes: ["Swablu-Stacona"],
		formeOrder: ["Swablu", "Swablu-Stacona"],
	},
	swablustacona: {
		num: 333,
		name: "Swablu-Stacona",
		baseSpecies: "Swablu",
		forme: "Stacona",
		types: ["Fairy", "Flying"],
		baseStats: {hp: 45, atk: 40, def: 60, spa: 40, spd: 75, spe: 50},
		abilities: {0: "Cute Charm", 1: "Pixilate", H: "Misty Surge"},
		heightm: 0.4,
		weightkg: 1.2,
		color: "Pink",
		evos: ["Altaria-Stacona"],
		eggGroups: ["Flying", "Dragon"],
	},
	altaria: {
		inherit: true,
		otherFormes: ["Altaria-Stacona"],
		formeOrder: ["Altaria", "Altaria-Stacona"],
	},
	altariastacona: {
		num: 334,
		name: "Altaria-Stacona",
		baseSpecies: "Altaria",
		forme: "Stacona",
		types: ["Fairy", "Flying"],
		baseStats: {hp: 75, atk: 60, def: 80, spa: 100, spd: 105, spe: 80},
		abilities: {0: "Cute Charm", 1: "Pixilate", H: "Misty Surge"},
		heightm: 1.1,
		weightkg: 20.6,
		color: "Pink",
		prevo: "Swablu-Stacona",
		eggGroups: ["Flying", "Dragon"],
	},
	togepi: {
		inherit: true,
		otherFormes: ["Togepi-Stacona"],
		formeOrder: ["Togepi", "Togepi-Stacona"],
	},
	togepistacona: {
		num: 175,
		name: "Togepi-Stacona",
		baseSpecies: "Togepi",
		forme: "Stacona",
		types: ["Flying"],
		baseStats: {hp: 20, atk: 20, def: 65, spa: 35, spd: 65, spe: 40},
		abilities: {0: "Cloud Nine", H: "Trace"},
		heightm: 0.3,
		weightkg: 1.5,
		color: "White",
		evos: ["Togetic-Stacona"],
		eggGroups: ["Undiscovered"],
	},
	togetic: {
		inherit: true,
		otherFormes: ["Togetic-Stacona"],
		formeOrder: ["Togetic", "Togetic-Stacona"],
	},
	togeticstacona: {
		num: 176,
		name: "Togetic-Stacona",
		baseSpecies: "Togetic",
		forme: "Stacona",
		types: ["Flying"],
		baseStats: {hp: 40, atk: 85, def: 40, spa: 105, spd: 55, spe: 80},
		abilities: {0: "Cloud Nine", H: "Trace"},
		heightm: 0.6,
		weightkg: 3.2,
		color: "White",
		evos: ["Togekiss-Stacona"],
		prevo: "Togepi-Stacona",
		eggGroups: ["Flying", "Fairy"],
	},
	togekiss: {
		inherit: true,
		otherFormes: ["Togekiss-Stacona"],
		formeOrder: ["Togekiss", "Togekiss-Stacona"],
	},
	togekissstacona: {
		num: 468,
		name: "Togekiss-Stacona",
		baseSpecies: "Togekiss",
		forme: "Stacona",
		types: ["Flying"],
		baseStats: {hp: 90, atk: 60, def: 90, spa: 110, spd: 100, spe: 95},
		abilities: {0: "Forecast"},
		heightm: 1.5,
		weightkg: 38,
		color: "White",
		prevo: "Togetic-Stacona",
		eggGroups: ["Flying", "Fairy"],
		otherFormes: ["Togekiss-Sunny", "Togekiss-Rainy", "Togekiss-Snowy"],
		formeOrder: ["Togekiss", "Togekiss-Sunny", "Togekiss-Rainy", "Togekiss-Snowy", "Togekiss-Sandy"],
	},
	togekissstaconasunny: {
		num: 468,
		name: "Togekis-Stacona-Sunny",
		baseSpecies: "Togekiss-Stacona",
		forme: "Sunny",
		types: ["Flying", "Fire"],
		baseStats: {hp: 90, atk: 60, def: 90, spa: 110, spd: 100, spe: 95},
		abilities: {0: "Forecast"},
		heightm: 1.5,
		weightkg: 38,
		color: "White",
		eggGroups: ["Flying", "Fairy"],
		requiredAbility: "Forecast",
	//	battleOnly: "Castform",
	},
	togekissstaconarainy: {
		num: 468,
		name: "Togekis-Stacona-Rainy",
		baseSpecies: "Togekiss-Stacona",
		forme: "Rainy",
		types: ["Flying", "Water"],
		baseStats: {hp: 90, atk: 60, def: 90, spa: 110, spd: 100, spe: 95},
		abilities: {0: "Forecast"},
		heightm: 1.5,
		weightkg: 38,
		color: "White",
		eggGroups: ["Flying", "Fairy"],
		requiredAbility: "Forecast",
	//	battleOnly: "Castform",
	},
	togekissstaconasnowy: {
		num: 468,
		name: "Togekis-Stacona-Snowy",
		baseSpecies: "Togekiss-Stacona",
		forme: "Snowy",
		types: ["Flying", "Ice"],
		baseStats: {hp: 90, atk: 60, def: 90, spa: 110, spd: 100, spe: 95},
		abilities: {0: "Forecast"},
		heightm: 1.5,
		weightkg: 38,
		color: "White",
		eggGroups: ["Flying", "Fairy"],
		requiredAbility: "Forecast",
	//	battleOnly: "Castform",
	},
	togekissstaconasandy: {
		num: 468,
		name: "Togekis-Stacona-Sandy",
		baseSpecies: "Togekiss-Stacona",
		forme: "Sandy",
		types: ["Flying", "Rock"],
		baseStats: {hp: 90, atk: 60, def: 90, spa: 110, spd: 100, spe: 95},
		abilities: {0: "Forecast"},
		heightm: 1.5,
		weightkg: 38,
		color: "White",
		eggGroups: ["Flying", "Fairy"],
		requiredAbility: "Forecast",
	//	battleOnly: "Castform",
	},
	dragaia: {
		num: 50026,
		name: "Dragaia",
		baseForme: "Normal",
		types: ["Normal", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		otherFormes: ["Dragaia-Bug", "Dragaia-Dark", "Dragaia-Dragon", "Dragaia-Electric", "Dragaia-Fairy", "Dragaia-Fighting", "Dragaia-Fire", "Dragaia-Flying", "Dragaia-Ghost", "Dragaia-Grass", "Dragaia-Ground", "Dragaia-Ice", "Dragaia-Poison", "Dragaia-Psychic", "Dragaia-Rock", "Dragaia-Steel", "Dragaia-Water"],
		formeOrder: [
			"Dragaia", "Dragaia-Fire", "Dragaia-Water", "Dragaia-Grass", "Dragaia-Flying", "Dragaia-Steel", "Dragaia-Electric", "Dragaia-Ice", "Dragaia-Psychic",
			"Dragaia-Bug", "Dragaia-Ground", "Dragaia-Poison", "Dragaia-Dragon", "Dragaia-Fighting", "Dragaia-Rock", "Dragaia-Ghost", "Dragaia-Dark", "Dragaia-Fairy",
		],

	},
	dragaiafire: {
		num: 50026,
		name: "Dragaia-Fire",
		baseSpecies: "Dragaia",
		forme: "Fire",
		types: ["Fire", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fire Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiawater: {
		num: 50026,
		name: "Dragaia-Water",
		baseSpecies: "Dragaia",
		forme: "Water",
		types: ["Water", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Water Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiagrass: {
		num: 50026,
		name: "Dragaia-Grass",
		baseSpecies: "Dragaia",
		forme: "Grass",
		types: ["Grass", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Grass Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiaflying: {
		num: 50026,
		name: "Dragaia-Flying",
		baseSpecies: "Dragaia",
		forme: "Flying",
		types: ["Flying", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Flying Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiasteel: {
		num: 50026,
		name: "Dragaia-Steel",
		baseSpecies: "Dragaia",
		forme: "Steel",
		types: ["Steel", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Steel Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiaelectric: {
		num: 50026,
		name: "Dragaia-Electric",
		baseSpecies: "Dragaia",
		forme: "Electric",
		types: ["Electric", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Electric Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiaice: {
		num: 50026,
		name: "Dragaia-Ice",
		baseSpecies: "Dragaia",
		forme: "Ice",
		types: ["Ice", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ice Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiapsychic: {
		num: 50026,
		name: "Dragaia-Psychic",
		baseSpecies: "Dragaia",
		forme: "Psychic",
		types: ["Psychic", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Psychic Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiabug: {
		num: 50026,
		name: "Dragaia-Bug",
		baseSpecies: "Dragaia",
		forme: "Bug",
		types: ["Bug", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Bug Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiaground: {
		num: 50026,
		name: "Dragaia-Ground",
		baseSpecies: "Dragaia",
		forme: "Ground",
		types: ["Ground", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ground Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiapoison: {
		num: 50026,
		name: "Dragaia-Poison",
		baseSpecies: "Dragaia",
		forme: "Poison",
		types: ["Poison", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Poison Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiadragon: {
		num: 50026,
		name: "Dragaia-Pure-Dragon",
		baseSpecies: "Dragaia",
		forme: "Pure Dragon",
		types: ["Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dragon Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiafighting: {
		num: 50026,
		name: "Dragaia-Fighting",
		baseSpecies: "Dragaia",
		forme: "Fighting",
		types: ["Fighting", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fighting Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiarock: {
		num: 50026,
		name: "Dragaia-Rock",
		baseSpecies: "Dragaia",
		forme: "Rock",
		types: ["Rock", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Rock Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiaghost: {
		num: 50026,
		name: "Dragaia-Ghost",
		baseSpecies: "Dragaia",
		forme: "Ghost",
		types: ["Ghost", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ghost Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiadark: {
		num: 50026,
		name: "Dragaia-Dark",
		baseSpecies: "Dragaia",
		forme: "Dark",
		types: ["Dark", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dark Cellulose"],
		changesFrom: "Dragaia",
	},
	dragaiafairy: {
		num: 50026,
		name: "Dragaia-Fairy",
		baseSpecies: "Dragaia",
		forme: "Fairy",
		types: ["Fairy", "Dragon"],
		gender: "N",
		baseStats: {hp: 98, atk: 120, def: 75, spa: 100, spd: 85, spe: 120},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Green",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fairy Cellulose"],
		changesFrom: "Dragaia",
	},
	prismatrix: {
		num: 50027,
		name: "Prismatrix",
		baseForme: "Normal",
		types: ["Normal", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		otherFormes: ["Prismatrix-Bug", "Prismatrix-Dark", "Prismatrix-Dragon", "Prismatrix-Electric", "Prismatrix-Fairy", "Prismatrix-Fighting", "Prismatrix-Fire", "Prismatrix-Flying", "Prismatrix-Ghost", "Prismatrix-Grass", "Prismatrix-Ground", "Prismatrix-Ice", "Prismatrix-Poison", "Prismatrix-Psychic", "Prismatrix-Rock", "Prismatrix-Steel", "Prismatrix-Water"],
		formeOrder: [
			"Prismatrix", "Prismatrix-Fire", "Prismatrix-Water", "Prismatrix-Grass", "Prismatrix-Flying", "Prismatrix-Steel", "Prismatrix-Electric", "Prismatrix-Ice", "Prismatrix-Psychic",
			"Prismatrix-Bug", "Prismatrix-Ground", "Prismatrix-Poison", "Prismatrix-Dragon", "Prismatrix-Fighting", "Prismatrix-Rock", "Prismatrix-Ghost", "Prismatrix-Dark", "Prismatrix-Fairy",
		],

	},
	prismatrixfire: {
		num: 50027,
		name: "Prismatrix-Fire",
		baseSpecies: "Prismatrix",
		forme: "Fire",
		types: ["Fire", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fire Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixwater: {
		num: 50027,
		name: "Prismatrix-Water",
		baseSpecies: "Prismatrix",
		forme: "Water",
		types: ["Water", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Water Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixgrass: {
		num: 50027,
		name: "Prismatrix-Grass",
		baseSpecies: "Prismatrix",
		forme: "Grass",
		types: ["Grass", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Grass Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixflying: {
		num: 50027,
		name: "Prismatrix-Flying",
		baseSpecies: "Prismatrix",
		forme: "Flying",
		types: ["Flying", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Flying Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixsteel: {
		num: 50027,
		name: "Prismatrix-Pure-Steel",
		baseSpecies: "Prismatrix",
		forme: "Pure Steel",
		types: ["Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Steel Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixelectric: {
		num: 50027,
		name: "Prismatrix-Electric",
		baseSpecies: "Prismatrix",
		forme: "Electric",
		types: ["Electric", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Electric Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixice: {
		num: 50027,
		name: "Prismatrix-Ice",
		baseSpecies: "Prismatrix",
		forme: "Ice",
		types: ["Ice", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ice Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixpsychic: {
		num: 50027,
		name: "Prismatrix-Psychic",
		baseSpecies: "Prismatrix",
		forme: "Psychic",
		types: ["Psychic", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Psychic Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixbug: {
		num: 50027,
		name: "Prismatrix-Bug",
		baseSpecies: "Prismatrix",
		forme: "Bug",
		types: ["Bug", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Bug Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixground: {
		num: 50027,
		name: "Prismatrix-Ground",
		baseSpecies: "Prismatrix",
		forme: "Ground",
		types: ["Ground", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ground Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixpoison: {
		num: 50027,
		name: "Prismatrix-Poison",
		baseSpecies: "Prismatrix",
		forme: "Poison",
		types: ["Poison", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Poison Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixdragon: {
		num: 50027,
		name: "Prismatrix-Dragon",
		baseSpecies: "Prismatrix",
		forme: "Dragon",
		types: ["Dragon", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dragon Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixfighting: {
		num: 50027,
		name: "Prismatrix-Fighting",
		baseSpecies: "Prismatrix",
		forme: "Fighting",
		types: ["Fighting", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fighting Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixrock: {
		num: 50027,
		name: "Prismatrix-Rock",
		baseSpecies: "Prismatrix",
		forme: "Rock",
		types: ["Rock", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Rock Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixghost: {
		num: 50027,
		name: "Prismatrix-Ghost",
		baseSpecies: "Prismatrix",
		forme: "Ghost",
		types: ["Ghost", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Ghost Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixdark: {
		num: 50027,
		name: "Prismatrix-Dark",
		baseSpecies: "Prismatrix",
		forme: "Dark",
		types: ["Dark", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Dark Cellulose"],
		changesFrom: "Prismatrix",
	},
	prismatrixfairy: {
		num: 50027,
		name: "Prismatrix-Fairy",
		baseSpecies: "Prismatrix",
		forme: "Fairy",
		types: ["Fairy", "Steel"],
		gender: "N",
		baseStats: {hp: 98, atk: 85, def: 120, spa: 120, spd: 100, spe: 85},
		abilities: {0: "Splice"},
		heightm: 7.0,
		weightkg: 206.5,
		color: "Gray",
		eggGroups: ["Undiscovered"],
		requiredItems: ["Fairy Cellulose"],
		changesFrom: "Prismatrix",
	},
	rocc: {
		num: 50028,
		name: "Rocc",
		types: ["Rock"],
		gender: "N",
		baseStats: {hp: 100, atk: 100, def: 200, spa: 50, spd: 50, spe: 30},
		abilities: {0: "Intimidate"},
		heightm: 1.5,
		weightkg: 420,
		color: "Brown",
		eggGroups: ["Undiscovered"],
	},
	yanmite: {
		num: 500029,
		name: "Yanmite",
		types: ["Bug", "Dragon"],
		baseStats: {hp: 35, atk: 50, def: 35, spa: 50, spd: 35, spe: 35},
		abilities: {0: "Multiscale", 1: "Mold Breaker", H: "Adaptability"},
		heightm: 0.3,
		weightkg: 1.5,
		color: "Green",
		evos: ["Yanma-Stacona"],
		eggGroups: ["Bug"],
	},
        yanma: {
		inherit: true,
		otherFormes: ["Yanma-Stacona"],
		formeOrder: ["Yanma", "Yanma-Stacona"],
	},
	yanmastacona: {
		num: 193,
		name: "Yanma-Stacona",
		baseSpecies: "Yanma",
		forme: "Stacona",
		types: ["Bug", "Dragon"],
		baseStats: {hp: 65, atk: 65, def: 45, spa: 75, spd: 45, spe: 95},
		abilities: {0: "Multiscale", 1: "Mold Breaker", H: "Adaptability"},
		heightm: 0.9,
		weightkg: 10,
		color: "Green",
		prevo: "Yanmite",
		evos: ["Yanmega-Stacona"],
		eggGroups: ["Bug"],
	},
        yanmega: {
		inherit: true,
		otherFormes: ["Yanmega-Stacona"],
		formeOrder: ["Yanmega", "Yanmega-Stacona"],
	},
	yanmegastacona: {
		num: 469,
		name: "Yanmega-Stacona",
		baseSpecies: "Yanmega",
		forme: "Stacona",
		types: ["Bug", "Dragon"],
		baseStats: {hp: 65, atk: 65, def: 45, spa: 75, spd: 45, spe: 95},
		abilities: {0: "Multiscale", 1: "Mold Breaker", H: "Adaptability"},
		heightm: 0.9,
		weightkg: 10,
		color: "Green",
		prevo: "Yanma-Stacona",
		eggGroups: ["Bug"],
	},
        hoppip: {
		inherit: true,
		otherFormes: ["Hoppip-Stacona"],
		formeOrder: ["Hoppip", "Hoppip-Stacona"],
	},
	hoppipstacona: {
		num: 187,
		name: "Hoppip-Stacona",
		baseSpecies: "Hoppip",
		forme: "Stacona",
		types: ["Ghost", "Grass"],
		baseStats: {hp: 55, atk: 100, def: 45, spa: 45, spd: 45, spe: 10},
		abilities: {0: "Cursed Body", 1: "Levitate", H: "Harvest"},
		heightm: 0.4,
		weightkg: 0.5,
		color: "Red",
		evos: ["Skiploom-Stacona"],
		eggGroups: ["Fairy", "Grass"],
	},
	skiploom: {
		inherit: true,
		otherFormes: ["Skiploom-Stacona"],
		formeOrder: ["Skiploom", "Skiploom-Stacona"],
	},
	skiploomstacona: {
		num: 188,
		name: "Skiploom-Stacona",
		baseSpecies: "Skiploom",
		forme: "Stacona",
		types: ["Ghost", "Grass"],
		baseStats: {hp: 55, atk: 45, def: 50, spa: 45, spd: 65, spe: 80},
		abilities: {0: "Cursed Body", 1: "Levitate", H: "Harvest"},
		heightm: 0.6,
		weightkg: 1,
		color: "Red",
		evos: ["Jumpluff-Stacona"],
		prevo: "Hoppip-Stacona",
		eggGroups: ["Fairy", "Grass"],
	},
	jumpluff: {
		inherit: true,
		otherFormes: ["Jumpluff-Stacona"],
		formeOrder: ["Jumpluff", "Jumpluff-Stacona"],
	},
	jumpluffstacona: {
		num: 189,
		name: "Jumpluff-Stacona",
		baseSpecies: "Jumpluff",
		forme: "Stacona",
		types: ["Ghost", "Grass"],
		baseStats: {hp: 75, atk: 55, def: 70, spa: 55, spd: 95, spe: 110},
		abilities: {0: "Cursed Body", 1: "Levitate", H: "Harvest"},
		heightm: 0.8,
		weightkg: 3,
		color: "Red",
		prevo: "Skiploom-Stacona",
		eggGroups: ["Fairy", "Grass"],
	},
        sunkern: {
		inherit: true,
		otherFormes: ["Sunkern-Stacona"],
		formeOrder: ["Sunkern", "Sunkern-Stacona"],
	},
	sunkernstacona: {
		num: 187,
		name: "Sunkern-Stacona",
		baseSpecies: "Sunkern",
		forme: "Stacona",
		types: ["Grass", "Dark"],
		baseStats: {hp: 30, atk: 30, def: 30, spa: 30, spd: 30, spe: 30},
		abilities: {0: "Leaf Guard", 1: "Cursed Body", H: "Poison Heal"},
		heightm: 0.3,
		weightkg: 1.8,
		color: "Yellow",
		evos: ["Sunflora-Stacona"],
		eggGroups: ["Grass"],
	},
        sunflora: {
		inherit: true,
		otherFormes: ["Sunflora-Stacona"],
		formeOrder: ["Sunflora", "Sunflora-Stacona"],
	},
	sunflorastacona: {
		num: 188,
		name: "Sunflora-Stacona",
		baseSpecies: "Sunflora",
		forme: "Stacona",
		types: ["Grass", "Dark"],
		baseStats: {hp: 100, atk: 30, def: 80, spa: 95, spd: 110, spe: 30},
		abilities: {0: "Leaf Guard", 1: "Cursed Body", H: "Poison Heal"},
		heightm: 0.8,
		weightkg: 8.5,
		color: "Yellow",
		prevo: "Sunkern-Stacona",
		eggGroups: ["Grass"],
	},
        natu: {
		inherit: true,
		otherFormes: ["Natu-Stacona"],
		formeOrder: ["Natu", "Natu-Stacona"],
	},
	natustacona: {
		num: 177,
		name: "Natu-Stacona",
		baseSpecies: "Natu",
		forme: "Stacona",
		types: ["Psychic", "Ice"],
		baseStats: {hp: 20, atk: 45, def: 25, spa: 110, spd: 25, spe: 95},
		abilities: {0: "Wonder Guard"},
		heightm: 0.2,
		weightkg: 2,
		color: "White",
		evos: ["Xatu-Stacona"],
		eggGroups: ["Flying"],
	},
        xatu: {
		inherit: true,
		otherFormes: ["Xatu-Stacona"],
		formeOrder: ["Xatu", "Xatu-Stacona"],
	},
	xatustacona: {
		num: 178,
		name: "Xatu-Stacona",
		baseSpecies: "Xatu",
		forme: "Stacona",
		types: ["Psychic", "Ice"],
		baseStats: {hp: 35, atk: 95, def: 30, spa: 155, spd: 30, spe: 125},
		abilities: {0: "Wonder Guard"},
		heightm: 1.5,
		weightkg: 15,
		color: "White",
		prevo: "Natu-Stacona",
		eggGroups: ["Flying"],
	},
        tyrogue: {
		inherit: true,
		otherFormes: ["Tyrogue-Stacona"],
		formeOrder: ["Tyrogue", "Tyrogue-Stacona"],
	},
        tyroguestacona: {
		num: 236,
		name: "Tyrogue-Stacona",
		baseSpecies: "Tyrogue",
		forme: "Stacona",
		types: ["Fighting", "Grass"],
		baseStats: {hp: 30, atk: 55, def: 55, spa: 30, spd: 30, spe: 10},
		abilities: {0: "Keen Eye", 1: "Rough Skin", H: "Storm Drain"},
		heightm: 0.7,
		weightkg: 21,
		color: "Green",
		evos: ["Hitmonlee-Stacona", "Hitmonchan-Stacona", "Hitmontop-Stacona"],
		eggGroups: ["Undiscovered"],
	},
	hitmonlee: {
		inherit: true,
		otherFormes: ["Hitmonlee-Stacona"],
		formeOrder: ["Hitmonlee", "Hitmonlee-Stacona"],
	},
        hitmonleestacona: {
		num: 106,
		name: "Hitmonlee-Stacona",
		baseSpecies: "Hitmonlee",
		forme: "Stacona",
		types: ["Fighting", "Grass"],
		baseStats: {hp: 60, atk: 130, def: 60, spa: 50, spd: 85, spe: 75},
		abilities: {0: "Moxie", 1: "Rough Skin", H: "Storm Drain"},
		heightm: 1.5,
		weightkg: 49.8,
		color: "Green",
		prevo: "Tyrogue-Stacona",
		eggGroups: ["Human-Like"],
	},
	hitmonchan: {
		inherit: true,
		otherFormes: ["Hitmonchan-Stacona"],
		formeOrder: ["Hitmonchan", "Hitmonchan-Stacona"],
	},
	hitmonchanstacona: {
		num: 107,
		name: "Hitmonchan-Stacona",
		baseSpecies: "Hitmonchan",
		forme: "Stacona",
		types: ["Fighting", "Grass"],
		baseStats: {hp: 60, atk: 70, def: 130, spa: 50, spd: 85, spe: 60},
		abilities: {0: "Moxie", 1: "Rough Skin", H: "Storm Drain"},
		heightm: 1.4,
		weightkg: 50.2,
		color: "Green",
		prevo: "Tyrogue-Stacona",
		eggGroups: ["Human-Like"],
	},
	hitmontop: {
		inherit: true,
		otherFormes: ["Hitmontop-Stacona"],
		formeOrder: ["Hitmontop", "Hitmontop-Stacona"],
	},
	hitmontopstacona: {
		num: 237,
		name: "Hitmontop-Stacona",
		baseSpecies: "Hitmontop",
		forme: "Stacona",
		types: ["Fighting", "Grass"],
		baseStats: {hp: 60, atk: 90, def: 90, spa: 50, spd: 85, spe: 80},
		abilities: {0: "Technician", 1: "Steadfast", H: "Storm Drain"},
		heightm: 1.4,
		weightkg: 50.2,
		color: "Green",
		prevo: "Tyrogue-Stacona",
		eggGroups: ["Human-Like"],
	},
	espurr: {
		inherit: true,
		otherFormes: ["Espurr-Stacona"],
		formeOrder: ["Espurr", "Espurr-Stacona"],
	},
	espurrstacona: {
		num: 677,
		name: "Espurr-Stacona",
		baseSpecies: "Espurr",
		forme: "Stacona",
		types: ["Dark", "Psychic"],
		baseStats: {hp: 62, atk: 48, def: 54, spa: 63, spd: 60, spe: 68},
		abilities: {0: "Keen Eye", 1: "Infiltrator", H: "Own Tempo"},
		heightm: 0.3,
		weightkg: 3.5,
		color: "Black",
		evos: ["Meowstic-Stacona"],
		eggGroups: ["Field"],
	},
        meowstic: {
		inherit: true,
		otherFormes: ["Meowstic-Stacona"],
		formeOrder: ["Meowstic", "Meowstic-Stacona"],
	},
	meowsticstacona: {
		num: 678,
		name: "Meowstic-Stacona",
		baseSpecies: "Meowstic",
		forme: "Stacona",
		types: ["Dark", "Psychic"],
		baseStats: {hp: 74, atk: 48, def: 76, spa: 83, spd: 81, spe: 104},
		abilities: {0: "Keen Eye", 1: "Infiltrator", H: "Competitive"},
		heightm: 0.6,
		weightkg: 8.5,
		color: "Black",
		prevo: "Espurr-Stacona",
		eggGroups: ["Field"],
	},
	budew: {
		inherit: true,
		otherFormes: ["Budew-Stacona"],
		formeOrder: ["Budew", "Budew-Stacona"],
	},
	budewstacona: {
		num: 406,
		name: "Budew-Stacona",
		baseSpecies: "Budew",
		forme: "Stacona",
		types: ["Grass", "Fire"],
		baseStats: {hp: 40, atk: 30, def: 35, spa: 70, spd: 70, spe: 55},
		abilities: {0: "Flame Body", 1: "Chloropyll", H: "Flash Fire"},
		heightm: 0.2,
		weightkg: 1.2,
		color: "Grass",
		evos: ["Roselia-Stacona"],
		eggGroups: ["Undiscovered"],
	},
	roselia: {
		inherit: true,
		otherFormes: ["Roselia-Stacona"],
		formeOrder: ["Roselia", "Roselia-Stacona"],
	},
	roseliastacona: {
		num: 315,
		name: "Roselia-Stacona",
		baseSpecies: "Roselia",
		forme: "Stacona",
		types: ["Grass", "Fire"],
		baseStats: {hp: 50, atk: 60, def: 45, spa: 100, spd: 100, spe: 65},
		abilities: {0: "Flame Body", 1: "Chloropyll", H: "Flash Fire"},
		heightm: 0.3,
		weightkg: 2,
		color: "Green",
		evos: ["Roserade-Stacona"],
		prevo: "Budew-Stacona",
		eggGroups: ["Fairy", "Grass"],
	},
	roserade: {
		inherit: true,
		otherFormes: ["Roserade-Stacona"],
		formeOrder: ["Roserade", "Roserade-Stacona"],
	},
	roseradestacona: {
		num: 407,
		name: "Roserade-Stacona",
		baseSpecies: "Roserade",
		forme: "Stacona",
		types: ["Grass", "Fire"],
		baseStats: {hp: 70, atk: 70, def: 55, spa: 135, spd: 125, spe: 90},
		abilities: {0: "Flame Body", 1: "Chloropyll", H: "Flash Fire"},
		heightm: 0.9,
		weightkg: 14.5,
		color: "Green",
		prevo: "Roselia-Stacona",
		eggGroups: ["Fairy", "Grass"],
	},
	paras: {
		inherit: true,
		otherFormes: ["Paras-Stacona"],
		formeOrder: ["Paras", "Paras-Stacona"],
	},
	parasstacona: {
		num: 46,
		name: "Paras-Stacona",
		baseSpecies: "Paras",
		forme: "Stacona",
		types: ["Bug", "Fairy"],
		baseStats: {hp: 35, atk: 70, def: 55, spa: 55, spd: 55, spe: 25},
		abilities: {0: "Effect Spore", 1: "Dry Skin", H: "Damp"},
		heightm: 0.3,
		weightkg: 5.4,
		color: "Pink",
		evos: ["Parasect-Stacona"],
		eggGroups: ["Bug", "Grass"],
	},
        parasect: {
		inherit: true,
		otherFormes: ["Parasect-Stacona"],
		formeOrder: ["Parasect", "Parasect-Stacona"],
	},
	parasectstacona: {
		num: 47,
		name: "Parasect-Stacona",
		baseSpecies: "Parasect",
		forme: "Stacona",
		types: ["Bug", "Fairy"],
		baseStats: {hp: 70, atk: 95, def: 90, spa: 90, spd: 80, spe: 40},
		abilities: {0: "Effect Spore", 1: "Dry Skin", H: "Damp"},
		heightm: 1,
		weightkg: 29.5,
		color: "Pink",
		prevo: "Paras-Stacona",
		eggGroups: ["Bug", "Grass"],
	},
	sentret: {
		inherit: true,
		otherFormes: ["Sentret-Stacona"],
		formeOrder: ["Sentret", "Sentret-Stacona"],
	},
	sentretstacona: {
		num: 161,
		name: "Sentret-Stacona",
		baseSpecies: "Sentret",
		forme: "Stacona",
		types: ["Normal", "Fairy"],
		baseStats: {hp: 34, atk: 35, def: 20, spa: 46, spd: 45, spe: 35},
		abilities: {0: "Cute Charm", 1: "Frisk", H: "Magic Guard"},
		heightm: 0.3,
		weightkg: 5.4,
		color: "Pink",
		evos: ["Furret-Stacona"],
		eggGroups: ["Field"],
	},
        furret: {
		inherit: true,
		otherFormes: ["Furret-Stacona"],
		formeOrder: ["Furret", "Furret-Stacona"],
	},
	furretstacona: {
		num: 162,
		name: "Furret-Stacona",
		baseSpecies: "Furret",
		forme: "Stacona",
		types: ["Normal", "Fairy"],
		baseStats: {hp: 74, atk: 45, def: 45, spa: 96, spd: 75, spe: 90},
		abilities: {0: "Cute Charm", 1: "Frisk", H: "Magic Guard"},
		heightm: 0.8,
		weightkg: 6,
		color: "Pink",
		prevo: "Sentret-Stacona",
		eggGroups: ["Field"],
	},
	aipom: {
		inherit: true,
		otherFormes: ["Aipom-Stacona"],
		formeOrder: ["Aipom", "Aipom-Stacona"],
	},
	aipomstacona: {
		num: 190,
		name: "Aipom-Stacona",
		baseSpecies: "Aipom",
		forme: "Stacona",
		types: ["Poison", "Fairy"],
		baseStats: {hp: 55, atk: 60, def: 65, spa: 70, spd: 65, spe: 55},
		abilities: {0: "Healer", 1: "Poison Touch", H: "Regenerator"},
		heightm: 0.8,
		weightkg: 11.5,
		color: "Purple",
		evos: ["Ambipom-Stacona"],
		eggGroups: ["Field"],
	},
        ambipom: {
		inherit: true,
		otherFormes: ["Ambipom-Stacona"],
		formeOrder: ["Ambipom", "Ambipom-Stacona"],
	},
	ambipomstacona: {
		num: 424,
		name: "Ambipom-Stacona",
		baseSpecies: "Ambipom",
		forme: "Stacona",
		types: ["Poison", "Fairy"],
		baseStats: {hp: 110, atk: 75, def: 71, spa: 105, spd: 71, spe: 60},
		abilities: {0: "Healer", 1: "Poison Touch", H: "Regenerator"},
		heightm: 1.2,
		weightkg: 20.3,
		color: "Purple",
		prevo: "Aipom-Stacona",
		eggGroups: ["Field"],
	},
	azurill: {
		inherit: true,
		otherFormes: ["Azurill-Stacona"],
		formeOrder: ["Azurill", "Azurill-Stacona"],
	},
	azurillstacona: {
		num: 298,
		name: "Azurill-Stacona",
		baseSpecies: "Azurill",
		forme: "Stacona",
		types: ["Fighting"],
		baseStats: {hp: 20, atk: 50, def: 50, spa: 20, spd: 45, spe: 5},
		abilities: {0: "Vital Spirit", 1: "Sturdy", H: "Keen Eye"},
		heightm: 0.2,
		weightkg: 18.2,
		color: "Brown",
		evos: ["Marill-Stacona"],
		eggGroups: ["Undiscovered"],
	},
	marill: {
		inherit: true,
		otherFormes: ["Marill-Stacona"],
		formeOrder: ["Marill", "Marill-Stacona"],
	},
	marillstacona: {
		num: 183,
		name: "Marill-Stacona",
		baseSpecies: "Marill",
		forme: "Stacona",
		types: ["Fighting"],
		baseStats: {hp: 55, atk: 70, def: 60, spa: 20, spd: 40, spe: 5},
		abilities: {0: "Vital Spirit", 1: "Sturdy", H: "Anger Point"},
		heightm: 0.4,
		weightkg: 25.4,
		color: "Brown",
		evos: ["Azumarill-Stacona"],
		prevo: "Azurill-Stacona",
		eggGroups: ["Water 1", "Fairy"],
	},
	azumarill: {
		inherit: true,
		otherFormes: ["Azumarill-Stacona"],
		formeOrder: ["Azumarill", "Azumarill-Stacona"],
	},
	azumarillstacona: {
		num: 184,
		name: "Azumarill-Stacona",
		baseSpecies: "Azumarill",
		forme: "Stacona",
		types: ["Fighting", "Rock"],
		baseStats: {hp: 90, atk: 110, def: 95, spa: 50, spd: 60, spe: 5},
		abilities: {0: "Solid Rock", 1: "Sturdy", H: "Anger Point"},
		heightm: 2,
		weightkg: 82,
		color: "Brown",
		prevo: "Marill-Stacona",
		eggGroups: ["Water 1", "Fairy"],
	},
	totodile: {
		inherit: true,
		otherFormes: ["Totodile-Stacona"],
		formeOrder: ["Totodile", "Totodile-Stacona"],
	},
	totodilestacona: {
		num: 158,
		name: "Totodile-Stacona",
		baseSpecies: "Totodile",
		forme: "Stacona",
		types: ["Rock", "Fire"],
		baseStats: {hp: 55, atk: 65, def: 60, spa: 40, spd: 55, spe: 45},
		abilities: {0: "Blaze", 1: "White Smoke", H: "Sheer Force", S: "Rock Head"},
		heightm: 0.6,
		weightkg: 9.5,
		color: "Yellow",
		evos: ["Croconaw-Stacona"],
		eggGroups: ["Monster"],
	},
	croconaw: {
		inherit: true,
		otherFormes: ["Croconaw-Stacona"],
		formeOrder: ["Croconaw", "Croconaw-Stacona"],
	},
	croconawstacona: {
		num: 159,
		name: "Croconaw-Stacona",
		baseSpecies: "Croconaw",
		forme: "Stacona",
		types: ["Rock", "Fire"],
		baseStats: {hp: 65, atk: 85, def: 80, spa: 50, spd: 65, spe: 65},
		abilities: {0: "Blaze", 1: "White Smoke", H: "Sheer Force", S: "Rock Head"},
		heightm: 1.1,
		weightkg: 25,
		color: "Yellow",
		evos: ["Feraligatr-Stacona"],
		prevo: "Totodile-Stacona",
		eggGroups: ["Monster"],
	},
	feraligatr: {
		inherit: true,
		otherFormes: ["Feraligatr-Stacona"],
		formeOrder: ["Feraligatr", "Feraligatr-Stacona"],
	},
	feraligatrstacona: {
		num: 160,
		name: "Feraligatr-Stacona",
		baseSpecies: "Feraligatr",
		forme: "Stacona",
		types: ["Rock", "Fire"],
		baseStats: {hp: 85, atk: 105, def: 100, spa: 70, spd: 85, spe: 85},
		abilities: {0: "Blaze", 1: "White Smoke", H: "Sheer Force", S: "Rock Head"},
		heightm: 2.3,
		weightkg: 88.8,
		color: "Yellow",
		prevo: "Croconaw-Stacona",
		eggGroups: ["Monster"],
	},
	onix: {
		inherit: true,
		otherFormes: ["Onix-Teldona"],
		formeOrder: ["Onix", "Onix-Teldona"],
	},
	onixteldona: {
		num: 95,
		name: "Onix-Teldona",
		baseSpecies: "Onix",
		forme: "Stacona",
		types: ["Ice", "Rock"],
		baseStats: {hp: 55, atk: 65, def: 130, spa: 40, spd: 55, spe: 50},
		abilities: {0: "Sturdy", 1: "Heatproof", H: "Sheer Force"},
		heightm: 8.8,
		weightkg: 210,
		color: "Blue",
		evos: ["Steelix-Teldona"],
		eggGroups: ["Mineral"],
	},
	steelix: {
		inherit: true,
		otherFormes: ["Steelix-Mega", "Steelix-Teldona"],
		formeOrder: ["Steelix", "Steelix-Mega", "Steelix-Teldona"],
	},
	steelixteldona: {
		num: 208,
		name: "Steelix-Teldona",
		baseSpecies: "Steelix",
		forme: "Teldona",
		types: ["Ice", "Steel"],
		baseStats: {hp: 85, atk: 105, def: 100, spa: 70, spd: 85, spe: 85},
		abilities: {0: "Sturdy", 1: "Heatproof", H: "Sheer Force"},
		heightm: 9.2,
		weightkg: 400,
		color: "Blue",
		prevo: "Onix-Teldona",
		eggGroups: ["Mineral"],
	},
	shuppet: {
		inherit: true,
		otherFormes: ["Shuppet-Stacona"],
		formeOrder: ["Shuppet", "Shuppet-Stacona"],
	},
	shuppetstacona: {
		num: 353,
		name: "Shuppet-Stacona",
		baseSpecies: "Shuppet",
		forme: "Stacona",
		types: ["Ghost", "Fairy"],
		baseStats: {hp: 50, atk: 55, def: 35, spa: 75, spd: 50, spe: 30},
		abilities: {0: "Cute Charm", 1: "Competitive", H: "Magic Bounce"},
		heightm: 0.6,
		weightkg: 2.3,
		color: "Pink",
		evos: ["Banette-Stacona"],
		eggGroups: ["Amorphous"],
	},
        banette: {
		inherit: true,
		otherFormes: ["Banette-Mega", "Banette-Stacona"],
		formeOrder: ["Banette", "Banette-Mega", "Banette-Stacona"],
	},
	banettestacona: {
		num: 354,
		name: "Banette-Stacona",
		baseSpecies: "Banette",
		forme: "Stacona",
		types: ["Ghost", "Fairy"],
		baseStats: {hp: 70, atk: 80, def: 60, spa: 110, spd: 60, spe: 70},
		abilities: {0: "Cute Charm", 1: "Competitive", H: "Magic Bounce"},
		heightm: 1.1,
		weightkg: 12.5,
		color: "Pink",
		prevo: "Shuppet-Stacona",
		eggGroups: ["Amorphous"],
	},
	ledyba: {
		inherit: true,
		otherFormes: ["Ledyba-Stacona"],
		formeOrder: ["Ledyba", "Ledyba-Stacona"],
	},
	ledybastacona: {
		num: 165,
		name: "Ledyba-Stacona",
		baseSpecies: "Ledyba",
		forme: "Stacona",
		types: ["Ground", "Fighting"],
		baseStats: {hp: 40, atk: 60, def: 40, spa: 30, spd: 50, spe: 55},
		abilities: {0: "Guts", 1: "Levitate", H: "Huge Power"},
		heightm: 1,
		weightkg: 10.8,
		color: "Brown",
		evos: ["Ledian-Stacona"],
		eggGroups: ["Bug"],
	},
        ledian: {
		inherit: true,
		otherFormes: ["Ledian-Stacona"],
		formeOrder: ["Ledian", "Ledian-Stacona"],
	},
	ledianstacona: {
		num: 166,
		name: "Ledian-Stacona",
		baseSpecies: "Ledian",
		forme: "Stacona",
		types: ["Ground", "Fighting"],
		baseStats: {hp: 75, atk: 110, def: 50, spa: 35, spd: 60, spe: 85},
		abilities: {0: "Guts", 1: "Levitate", H: "Huge Power"},
		heightm: 1.4,
		weightkg: 35.6,
		color: "Brown",
		prevo: "Ledyba-Stacona",
		eggGroups: ["Bug"],
	},
	durant: {
		inherit: true,
		otherFormes: ["Durant-Stacona"],
		formeOrder: ["Durant", "Durant-Stacona"],
	},
	durantstacona: {
		num: 632,
		name: "Durant-Stacona",
		baseSpecies: "Durant",
		forme: "Stacona",
		types: ["Ground", "Grass"],
		baseStats: {hp: 60, atk: 110, def: 110, spa: 50, spd: 48, spe: 110},
		abilities: {0: "Overgrow", 1: "Hustle"},
		heightm: 0.3,
		weightkg: 33,
		color: "Brown",
		eggGroups: ["Bug"],
	},
	unownstacona: {
		num: 50030,
		name: "Unown-Stacona",
		baseForme: "A",
		types: ["Psychic", "Rock"],
		baseStats: {hp: 75, atk: 75, def: 75, spa: 75, spd: 75, spe: 75},
		abilities: {0: "Levitate"},
		heightm: 0.5,
		weightkg: 5,
		color: "Brown",
		eggGroups: ["Undiscovered"],
		cosmeticFormes: ["Unown-Stacona-B", "Unown-Stacona-C", "Unown-Stacona-D", "Unown-Stacona-E", "Unown-Stacona-F", "Unown-Stacona-G", "Unown-Stacona-H", "Unown-Stacona-I", "Unown-Stacona-J", "Unown-Stacona-K", "Unown-Stacona-L", "Unown-Stacona-M", "Unown-Stacona-N", "Unown-Stacona-O", "Unown-Stacona-P", "Unown-Stacona-Q", "Unown-Stacona-R", "Unown-Stacona-S", "Unown-Stacona-T", "Unown-Stacona-U", "Unown-Stacona-V", "Unown-Stacona-W", "Unown-Stacona-X", "Unown-Stacona-Y", "Unown-Stacona-Z", "Unown-Stacona-Exclamation", "Unown-Stacona-Question"],
		formeOrder: ["Unown-Stacona", "Unown-Stacona-B", "Unown-Stacona-C", "Unown-Stacona-D", "Unown-Stacona-E", "Unown-Stacona-F", "Unown-Stacona-G", "Unown-Stacona-H", "Unown-Stacona-I", "Unown-Stacona-J", "Unown-Stacona-K", "Unown-Stacona-L", "Unown-Stacona-M", "Unown-Stacona-N", "Unown-Stacona-O", "Unown-Stacona-P", "Unown-Stacona-Q", "Unown-Stacona-R", "Unown-Stacona-S", "Unown-Stacona-T", "Unown-Stacona-U", "Unown-Stacona-V", "Unown-Stacona-W", "Unown-Stacona-X", "Unown-Stacona-Y", "Unown-Stacona-Z", "Unown-Stacona-Exclamation", "Unown-Stacona-Question"],
	},
};
